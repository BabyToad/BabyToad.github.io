(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const p of r.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&n(p)}).observe(document,{childList:!0,subtree:!0});function a(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(s){if(s.ep)return;s.ep=!0;const r=a(s);fetch(s.href,r)}})();function Ne(t){const e=t.getContext("2d");t.width=t.clientWidth*devicePixelRatio,t.height=t.clientHeight*devicePixelRatio,e.scale(devicePixelRatio,devicePixelRatio);const a=t.clientWidth,n=t.clientHeight;let s=a/2-20,r=n/2-20;const p=5,f={},x=v=>{"wasd".includes(v.key.toLowerCase())&&(f[v.key.toLowerCase()]=v.type==="keydown",v.preventDefault())};window.addEventListener("keydown",x),window.addEventListener("keyup",x);let R;function _(){f.a&&(s-=p),f.d&&(s+=p),f.w&&(r-=p),f.s&&(r+=p),s=Math.max(0,Math.min(s,a-40)),r=Math.max(0,Math.min(r,n-40)),e.fillStyle="#0b0d10",e.fillRect(0,0,a,n),e.fillStyle="#ff6b35",e.fillRect(s,r,40,40),R=requestAnimationFrame(_)}return _(),()=>{cancelAnimationFrame(R),window.removeEventListener("keydown",x),window.removeEventListener("keyup",x)}}const D=1200,B=540,le=140,Z=B+le,Q=42,H=54,Oe=7,We=8,De=.35,Fe=2.2,je=.5,Ue=1.55,qe=1.9,Y=240,re=[[0,470,340,70],[420,470,200,70],[680,420,160,120],[880,350,100,190],[1040,470,160,70],[920,280,80,16]],Me=`
.playground-wrap {
...
`;function Ge(){if(document.getElementById("mario-jump-styles"))return;const t=document.createElement("style");t.id="mario-jump-styles",t.textContent=Me,document.head.appendChild(t)}function He(t){Ge();const e=t.getContext("2d"),a=Math.max(1,Math.min(3,window.devicePixelRatio||1));let n=0,s=0;function r(){const i=t.getBoundingClientRect();i.width<1||i.height<1||(n=i.width,s=i.height,t.width=Math.round(n*a),t.height=Math.round(s*a))}const p=new ResizeObserver(r);p.observe(t),r();const f=new Image;f.src="img/claude-sprite.png";let x=!1;f.onload=()=>{x=!0};const R=new Image;R.src="img/playground-backdrop.png";let _=!1;R.onload=()=>{_=!0};const v={gravity:.65,jumpStrength:13.5,moveSpeed:5,airControl:1,realisticMode:!1,variableJump:!0,coyoteTime:!0,jumpBuffer:!0,apexHang:!0,asymmetricGravity:!0,fastFall:!0},k={...v},N=["variableJump","coyoteTime","jumpBuffer","apexHang","asymmetricGravity","fastFall"],o={x:80,y:470-H,vx:0,vy:0,facing:1,squash:1},l={onGround:!1,coyote:0,buffer:0,currentGravity:v.gravity,jumpPressedThisFrame:!1,releasedJumpInRise:!1},A={vy:new Float32Array(Y),ay:new Float32Array(Y),y:new Float32Array(Y),head:0};function c(){A.vy[A.head]=o.vy,A.ay[A.head]=l.currentGravity,A.y[A.head]=o.y,A.head=(A.head+1)%Y}function g(){o.x=80,o.y=470-H,o.vx=0,o.vy=0,o.facing=1,o.squash=1,l.onGround=!1,l.coyote=0,l.buffer=0,l.releasedJumpInRise=!1,A.vy.fill(0),A.ay.fill(0),A.y.fill(o.y),A.head=0}const u={left:!1,right:!1,jump:!1,down:!1},C={arrowleft:"left",a:"left",arrowright:"right",d:"right",arrowup:"jump",w:"jump"," ":"jump",arrowdown:"down",s:"down"};let b=!1;function F(i){if(!b)return;const y=i.key.toLowerCase();if(y==="r"&&i.type==="keydown"){g(),i.preventDefault(),i.stopImmediatePropagation();return}const m=C[y];if(!m)return;const T=i.type==="keydown";m==="jump"&&(T&&!u.jump&&(l.jumpPressedThisFrame=!0),!T&&u.jump&&o.vy<0&&(l.releasedJumpInRise=!0)),u[m]=T,i.preventDefault(),i.stopImmediatePropagation()}window.addEventListener("keydown",F,!0),window.addEventListener("keyup",F,!0),t.setAttribute("tabindex","0"),t.style.outline="none";const z=()=>{b=!0,t.style.boxShadow="inset 0 0 0 2px rgba(255,107,53,0.6)"},q=()=>{b=!1,t.style.boxShadow="",u.left=u.right=u.jump=u.down=!1};t.addEventListener("focus",z),t.addEventListener("blur",q);const X=()=>t.focus();t.addEventListener("pointerdown",X);function M(i,y,m){return`<label class="pg-toggle">
      <input type="checkbox" data-cfg="${i}" ${v[i]?"checked":""} />
      <span class="pg-toggle__label">${y}</span>
      <span class="pg-toggle__desc">${m}</span>
    </label>`}function K(i,y,m,T,S,$=E=>E.toFixed(2)){return`<label class="pg-slider">
      <span class="pg-slider__label">${S}</span>
      <span class="pg-slider__value" data-for="${i}">${$(v[i])}</span>
      <input type="range" data-cfg="${i}" min="${y}" max="${m}" step="${T}" value="${v[i]}" />
    </label>`}const L=document.createElement("div");L.className="playground-ui",L.innerHTML=`
    <div class="pg-section">
      <div class="pg-heading">Tricks</div>
      ${M("realisticMode","Realistic (no tricks)","Pure parabolic physics. Try this first — feel how committed it is.")}
      ${M("variableJump","Variable jump height","Release jump early → shorter jump.")}
      ${M("coyoteTime","Coyote time","Jump within ~7 frames of leaving a ledge.")}
      ${M("jumpBuffer","Jump buffer","Press jump ~8 frames before landing → still fires.")}
      ${M("apexHang","Apex hangtime","Near the peak, gravity halves. Floaty, controllable.")}
      ${M("asymmetricGravity","Asymmetric gravity","Falls faster than it rises. Snappy.")}
      ${M("fastFall","Fast fall","Hold ↓ or S in air → accelerates down.")}
    </div>
    <div class="pg-section">
      <div class="pg-heading">Tuning</div>
      ${K("gravity",.1,2,.05,"Gravity")}
      ${K("jumpStrength",5,22,.5,"Jump strength",i=>i.toFixed(1))}
      ${K("moveSpeed",1,10,.5,"Move speed",i=>i.toFixed(1))}
  ...
  `;const G={vx:L.querySelector("[data-stat=vx]"),vy:L.querySelector("[data-stat=vy]"),ay:L.querySelector("[data-stat=ay]"),ground:L.querySelector("[data-stat=ground]"),coyote:L.querySelector("[data-stat=coyote]"),buffer:L.querySelector("[data-stat=buffer]")};L.querySelectorAll("input[data-cfg]").forEach(i=>{i.addEventListener("input",()=>{const y=i.dataset.cfg;if(i.type==="checkbox"){if(k[y]=i.checked,y==="realisticMode"&&i.checked&&N.forEach(m=>{k[m]=!1;const T=L.querySelector(`input[data-cfg="${m}"]`);T&&(T.checked=!1)}),N.includes(y)&&i.checked&&k.realisticMode){k.realisticMode=!1;const m=L.querySelector('input[data-cfg="realisticMode"]');m&&(m.checked=!1)}}else{const m=parseFloat(i.value);k[y]=m;const T=L.querySelector(`.pg-slider__value[data-for="${y}"]`);T&&(T.textContent=Number.isInteger(m*2)?m.toFixed(1):m.toFixed(2))}})}),L.querySelector(".pg-reset").addEventListener("click",g);const ee=t.closest(".demo__split");ee.classList.add("playground-wrap"),ee.appendChild(L);function he(i,y,m,T,S,$,E,w){return i<S+E&&i+m>S&&y<$+w&&y+T>$}function ie(i,y,m){return i+(y-i)*m}function _e(i){let y=0;u.left&&(y-=k.moveSpeed),u.right&&(y+=k.moveSpeed);const m=l.onGround?1:k.airControl;o.vx=ie(o.vx,y,Math.min(1,.3*m*i)),Math.abs(y)>.2&&(o.facing=Math.sign(y)),l.jumpPressedThisFrame&&(l.buffer=k.jumpBuffer?We:1),l.buffer=Math.max(0,l.buffer-i);const T=l.onGround||k.coyoteTime&&l.coyote>0;l.buffer>0&&T&&(o.vy=-k.jumpStrength,l.buffer=0,l.coyote=0,l.onGround=!1,l.releasedJumpInRise=!1,o.squash=1.2),k.variableJump&&l.releasedJumpInRise&&o.vy<0&&(o.vy*=De,l.releasedJumpInRise=!1),u.jump||(l.releasedJumpInRise=!1);let S=k.gravity;const $=!k.realisticMode;$&&k.apexHang&&Math.abs(o.vy)<Fe&&(S*=je),$&&k.asymmetricGravity&&o.vy>0&&(S*=Ue),$&&k.fastFall&&u.down&&o.vy>0&&(S*=qe),l.currentGravity=S,o.vy+=S*i,o.vy>18&&(o.vy=18),o.x+=o.vx*i;for(const[w,I,P,j]of re)he(o.x,o.y,Q,H,w,I,P,j)&&(o.vx>0?o.x=w-Q:o.vx<0&&(o.x=w+P),o.vx=0);const E=l.onGround;l.onGround=!1,o.y+=o.vy*i;for(const[w,I,P,j]of re)if(he(o.x,o.y,Q,H,w,I,P,j)){if(o.vy>0){if(o.y=I-H,l.onGround=!0,!E){const J=Math.min(1,o.vy/16);o.squash=ie(1,.7,J)}}else o.vy<0&&(o.y=I+j);o.vy=0}E&&!l.onGround&&o.vy>=0?l.coyote=Oe:l.coyote>0&&!l.onGround?l.coyote=Math.max(0,l.coyote-i):l.onGround&&(l.coyote=0),o.squash=ie(o.squash,1,Math.min(1,.2*i)),(o.y>B+200||o.x<-300||o.x>D+300)&&g(),l.jumpPressedThisFrame=!1,c()}let ke=0;function xe(){if(++ke%4!==0)return;const i=y=>y>=0?"+":"";G.vx.textContent=`${i(o.vx)}${o.vx.toFixed(2)}`,G.vy.textContent=`${i(o.vy)}${o.vy.toFixed(2)}`,G.ay.textContent=`${i(l.currentGravity)}${l.currentGravity.toFixed(2)}`,G.ground.textContent=l.onGround?"yes":"no",G.ground.className=l.onGround?"good":"muted",G.coyote.textContent=l.coyote.toFixed(0),G.coyote.className=l.coyote>0?"accent":"muted",G.buffer.textContent=l.buffer.toFixed(0),G.buffer.className=l.buffer>0?"accent":"muted"}function Te(){if(_){const E=R.width/R.height,w=D/B;let I,P,j,J;E>w?(P=B,I=P*E,j=(D-I)/2,J=0):(I=D,P=I/E,j=0,J=(B-P)/2),e.drawImage(R,j,J,I,P)}else{const E=e.createLinearGradient(0,0,0,B);E.addColorStop(0,"#0b0d10"),E.addColorStop(1,"#2a1f1a"),e.fillStyle=E,e.fillRect(0,0,D,B)}for(const[E,w,I,P]of re)e.fillStyle="#b78363",e.fillRect(E,w,I,P),e.fillStyle="#f1c591",e.fillRect(E,w,I,5),e.fillStyle="rgba(0,0,0,0.18)",e.fillRect(E+I-3,w,3,P),e.fillStyle="#8a5a3a",e.fillRect(E,w+5,I,2);e.save();const i=o.x+Q/2,y=o.y+H;e.translate(i,y);let m=o.squash,T=1/o.squash;if(!l.onGround){const E=Math.max(-.15,Math.min(.22,-o.vy/80));m*=1+E,T*=1-E*.6}e.scale(T*o.facing,m);const S=Q*2,$=H*2;x?e.drawImage(f,-S/2,-$*.92,S,$):(e.fillStyle="#d97757",e.fillRect(-Q/2,-H,Q,H)),e.restore()}function Ae(){const i=B,y=12,m=3,T=12,S=(D-y*2-T*(m-1))/m,$=le-y*2;e.fillStyle="rgba(11, 13, 16, 0.85)",e.fillRect(0,i,D,le),e.fillStyle="rgba(255,255,255,0.06)",e.fillRect(0,i,D,1),[{label:"y (position)",data:A.y,color:"#6fb3d2",min:0,max:B,invert:!0,zeroLine:!1},{label:"vy (velocity)",data:A.vy,color:"#ff6b35",min:-22,max:22,invert:!1,zeroLine:!0},{label:"ay (accel/grav)",data:A.ay,color:"#98c379",min:0,max:3,invert:!1,zeroLine:!1}].forEach((w,I)=>{const P=y+I*(S+T);if(e.fillStyle="rgba(255,255,255,0.04)",e.fillRect(P,i+y,S,$),e.font="12px ui-monospace, monospace",e.textBaseline="top",e.fillStyle="rgba(255,255,255,0.55)",e.fillText(w.label,P+8,i+y+6),w.zeroLine){const V=i+y+$*((0-w.min)/(w.max-w.min));e.fillStyle="rgba(255,255,255,0.15)",e.fillRect(P,V,S,1)}e.strokeStyle=w.color,e.lineWidth=1.5,e.beginPath();for(let V=0;V<Y;V++){const Re=(A.head+V)%Y,$e=w.data[Re],Ce=V/(Y-1),ge=Math.max(0,Math.min(1,($e-w.min)/(w.max-w.min))),Le=w.invert?ge:1-ge,me=P+Ce*S,be=i+y+Le*$;V===0?e.moveTo(me,be):e.lineTo(me,be)}e.stroke();const j=(A.head-1+Y)%Y,J=w.data[j],ye=Math.max(0,Math.min(1,(J-w.min)/(w.max-w.min))),Se=w.invert?ye:1-ye,Pe=P+S-1,Ie=i+y+Se*$;e.fillStyle=w.color,e.beginPath(),e.arc(Pe,Ie,3,0,Math.PI*2),e.fill()})}function Ee(){if(n<1||s<1)return;e.setTransform(1,0,0,1,0,0),e.clearRect(0,0,t.width,t.height);const i=Math.min(n/D,s/Z),y=(n-D*i)/2,m=(s-Z*i)/2,T=i*a;e.setTransform(T,0,0,T,y*a,m*a),e.imageSmoothingEnabled=!1,Te(),Ae(),b||(e.fillStyle="rgba(0,0,0,0.45)",e.fillRect(0,0,D,Z),e.fillStyle="#ffe6d4",e.font="bold 28px ui-monospace, monospace",e.textAlign="center",e.textBaseline="middle",e.fillText("click to play",D/2,Z/2),e.textAlign="start")}let se,pe=performance.now();function ue(i){const y=Math.min(40,i-pe);pe=i;const m=y/(1e3/60);_e(m),xe(),Ee(),se=requestAnimationFrame(ue)}return se=requestAnimationFrame(ue),()=>{cancelAnimationFrame(se),window.removeEventListener("keydown",F,!0),window.removeEventListener("keyup",F,!0),t.removeEventListener("focus",z),t.removeEventListener("blur",q),t.removeEventListener("pointerdown",X),p.disconnect(),L.remove(),ee&&ee.classList.remove("playground-wrap"),t.style.boxShadow=""}}const Ye={"moving-square":Ne,"mario-jump":He};let te=null;function Be(t){if(te&&(te(),te=null),!t)return;const e=t.querySelector(".demo__canvas"),a=t.getAttribute("data-demo");if(!e||!a)return;const n=Ye[a];if(!n){console.warn(`[demo] unknown demo: ${a}`);return}te=n(e)||null}let ne=null;function ze(t){if(ne&&(ne(),ne=null),!(t!=null&&t.hasAttribute("data-interactive-code")))return;const e=t.querySelector(".icode");if(!e)return;const a=[...t.querySelectorAll(".icode__line[data-region]")],n=[...t.querySelectorAll(".icode__ann[data-region]")],s=[...t.querySelectorAll(".icode-tip")],r=[...new Set(a.map(c=>c.dataset.region).filter(Boolean))],p=new Set;let f=null,x=null,R=!1,_=null;function v(){const c=f!==null;e.classList.toggle("icode--has-active",c),e.classList.toggle("icode--has-revealed",p.size>0);for(const g of a){const u=g.dataset.region;u&&(g.classList.toggle("icode__line--active",u===f),g.classList.toggle("icode__line--revealed",p.has(u)))}for(const g of n){const u=g.dataset.region;g.classList.toggle("icode__ann--active",u===f),g.classList.toggle("icode__ann--revealed",p.has(u))}}function k(){R||p.size>=r.length&&(R=!0,e.classList.add("icode--complete"),setTimeout(()=>e.classList.remove("icode--complete"),900))}v();function N(c){var g,u;if(!((g=c.target)!=null&&g.isContentEditable||/input|textarea/i.test((u=c.target)==null?void 0:u.tagName))){if(c.key>="1"&&c.key<="9"){const C=parseInt(c.key)-1;if(C>=r.length)return;c.preventDefault();const b=r[C];f===b?(f=null,x=null):(p.add(b),f=b,x=b,k()),v();return}if(c.key==="0"||c.key==="Escape"){(f||x)&&(c.preventDefault(),f=null,x=null,v());return}}}addEventListener("keydown",N);const o=[];for(const c of a){const g=c.dataset.region;if(!g)continue;const u=()=>{!x&&p.has(g)&&(f=g,v())},C=()=>{x||(f=null,v())};c.addEventListener("mouseenter",u),c.addEventListener("mouseleave",C),o.push({el:c,events:{mouseenter:u,mouseleave:C}})}for(const c of n){const g=c.dataset.region,u=()=>{!x&&p.has(g)&&(f=g,v())},C=()=>{x||(f=null,v())},b=()=>{x===g?(x=null,f=null):(p.add(g),x=g,f=g,k()),v()};c.addEventListener("mouseenter",u),c.addEventListener("mouseleave",C),c.addEventListener("click",b),o.push({el:c,events:{mouseenter:u,mouseleave:C,click:b}})}function l(c){A();const g=c.dataset.tipSig||"",u=c.dataset.tipDesc||"",C=c.dataset.tipUrl||"";if(!g&&!u)return;const b=document.createElement("div");b.className="icode-tooltip",b.innerHTML=[g?`<code class="icode-tooltip__sig">${de(g)}</code>`:"",u?`<div class="icode-tooltip__desc">${de(u)}</div>`:"",C?`<a class="icode-tooltip__link" href="${de(C)}" target="_blank" rel="noopener">→ docs</a>`:""].filter(Boolean).join(""),e.appendChild(b),e.style.position="relative";let F=0,z=0,q=c;for(;q&&q!==e;)F+=q.offsetTop,z+=q.offsetLeft,q=q.offsetParent;b.style.left=Math.max(0,z)+"px",b.style.visibility="hidden";const X=b.offsetHeight,M=b.offsetWidth;F-X-10<0?(b.style.top=F+c.offsetHeight+8+"px",b.classList.add("icode-tooltip--below")):b.style.top=F-X-8+"px";const K=e.offsetWidth-M-8;parseInt(b.style.left)>K&&(b.style.left=Math.max(0,K)+"px"),b.style.visibility="",_=b}function A(){_&&(_.remove(),_=null)}for(const c of s){const g=()=>l(c),u=()=>A();c.addEventListener("mouseenter",g),c.addEventListener("mouseleave",u),o.push({el:c,events:{mouseenter:g,mouseleave:u}})}ne=()=>{removeEventListener("keydown",N);for(const c of o)for(const[g,u]of Object.entries(c.events))c.el.removeEventListener(g,u);A(),f=null,x=null,e.classList.remove("icode--has-active","icode--has-revealed","icode--complete");for(const c of a)c.classList.remove("icode__line--active","icode__line--revealed");for(const c of n)c.classList.remove("icode__ann--active","icode__ann--revealed")}}function de(t){return(t||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const Ke=localStorage.getItem("deck-theme")||"dark";document.documentElement.setAttribute("data-theme",Ke);class Je extends HTMLElement{connectedCallback(){if(this._wired)return;this._wired=!0;const e=document.createElement("div");for(e.className="deck__stage";this.firstChild;)e.appendChild(this.firstChild);this.appendChild(e),this._stage=e,this._collectSlides(),this._buildFooter(),this._buildHelp(),this._index=this._readHashIndex(),this._show(this._index),this._scale(),addEventListener("resize",()=>this._scale()),addEventListener("keydown",a=>this._onKey(a)),addEventListener("hashchange",()=>this._show(this._readHashIndex()))}_collectSlides(){const a=new URLSearchParams(location.search).get("unit");let n=Array.from(this._stage.querySelectorAll(".slide"));a&&(n=n.filter(s=>s.getAttribute("data-unit")===a),this._stage.querySelectorAll(".slide").forEach(s=>{n.includes(s)||s.remove()})),this._slides=n}_buildFooter(){const e=document.createElement("div");e.className="deck__footer",e.innerHTML=`
    ...
    `,document.body.appendChild(e),this._help=e}_readHashIndex(){var n;const e=location.hash.match(/^#(\d+)/);if(!e)return 0;const a=parseInt(e[1],10)-1;return Math.max(0,Math.min(a,(((n=this._slides)==null?void 0:n.length)||1)-1))}_show(e){if(!this._slides.length||e<0||e>=this._slides.length)return;this._index=e;const a=this._slides.length;this._slides.forEach((R,_)=>{_===e?R.setAttribute("data-active",""):R.removeAttribute("data-active")});const n=this._slides[e],s=n.getAttribute("data-unit")||"",r=n.getAttribute("data-tag")||"",p=n.getAttribute("data-total")||"12",f=[];s&&f.push(`UNIT ${s} / ${p}`),r&&f.push(r);const x=f.join("  —  ");this._fTag&&(this._fTag.style.animation="none",this._fTag.offsetHeight,this._fTag.style.animation="",this._fTag.textContent=x),this._fNum&&(this._fNum.textContent=`${String(e+1).padStart(2,"0")} / ${String(a).padStart(2,"0")}`),this._progress&&(this._progress.style.width=`${(e+1)/a*100}%`),location.hash!==`#${e+1}`&&history.replaceState(null,"",`#${e+1}`),Be(n),ze(n),dispatchEvent(new CustomEvent("deck:slide-change",{detail:{index:e,slide:n}}))}_next(){this._show(Math.min(this._index+1,this._slides.length-1))}_prev(){this._show(Math.max(this._index-1,0))}_onKey(e){if(!(e.target&&e.target.isContentEditable)&&!(e.target&&/input|textarea/i.test(e.target.tagName))){if(e.ctrlKey&&e.shiftKey&&(e.key==="E"||e.key==="e")){e.preventDefault(),dispatchEvent(new CustomEvent("deck:toggle-edit"));return}switch(e.key){case"ArrowRight":case" ":case"PageDown":e.preventDefault(),this._next();break;case"ArrowLeft":case"PageUp":e.preventDefault(),this._prev();break;case"Home":e.preventDefault(),this._show(0);break;case"End":e.preventDefault(),this._show(this._slides.length-1);break;case"f":case"F":document.fullscreenElement?document.exitFullscreen():document.documentElement.requestFullscreen();break;case"t":case"T":e.preventDefault(),this._toggleTheme();break;case"?":this._help.toggleAttribute("data-open");break}}}_toggleTheme(){const a=(document.documentElement.getAttribute("data-theme")||"dark")==="dark"?"light":"dark";document.documentElement.setAttribute("data-theme",a),localStorage.setItem("deck-theme",a)}_scale(){const e=innerWidth,a=innerHeight,n=Math.min(e/1920,a/1080);this._stage.style.transform=`translate(-50%, -50%) scale(${n})`}}customElements.get("deck-root")||customElements.define("deck-root",Je);const Ve=["B","I","EM","STRONG","BR"],Qe=new Set(Ve),Xe=new Set(["BR"]);function d(t){return t==null?"":String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function U(t){if(t==null)return"";const e=document.createElement("template");return e.innerHTML=String(t),ce(e.content)}function ce(t){let e="";for(const a of t.childNodes)if(a.nodeType===Node.TEXT_NODE)e+=d(a.nodeValue);else if(a.nodeType===Node.ELEMENT_NODE){const n=a.tagName;if(Qe.has(n)){const s=n.toLowerCase();Xe.has(n)?e+=`<${s}>`:e+=`<${s}>${ce(a)}</${s}>`}else e+=ce(a)}return e}const Ze=new Set(["False","None","True","and","as","assert","async","await","break","class","continue","def","del","elif","else","except","finally","for","from","global","if","import","in","is","lambda","nonlocal","not","or","pass","raise","return","try","while","with","yield"]),et=new Set(["print","len","range","str","int","float","list","dict","set","tuple","bool","type","isinstance","abs","min","max","sum","enumerate","zip","map","filter","input","open","round","self","cls"]),fe=new RegExp(["(?<comment>#[^\\n]*)",`(?<tstring>"""[\\s\\S]*?"""|'''[\\s\\S]*?''')`,`(?<fstring>f"(?:[^"\\\\\\n]|\\\\.)*"|f'(?:[^'\\\\\\n]|\\\\.)*')`,`(?<string>"(?:[^"\\\\\\n]|\\\\.)*"|'(?:[^'\\\\\\n]|\\\\.)*')`,"(?<decorator>@\\w+)","(?<number>\\b\\d+(?:\\.\\d+)?\\b)","(?<word>[A-Za-z_]\\w*)"].join("|"),"g");function O(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function ae(t){if(t==null)return"";let e="",a=0,n=null,s;for(fe.lastIndex=0;(s=fe.exec(t))!==null;){if(s.index>a){const p=t.slice(a,s.index);e+=O(p),/\S/.test(p)&&(n=null)}const r=s.groups;if(r.comment)e+=`<span class="tok-comment">${O(r.comment)}</span>`,n=null;else if(r.tstring)e+=`<span class="tok-string">${O(r.tstring)}</span>`,n=null;else if(r.fstring)e+=`<span class="tok-string">${O(r.fstring)}</span>`,n=null;else if(r.string)e+=`<span class="tok-string">${O(r.string)}</span>`,n=null;else if(r.decorator)e+=`<span class="tok-decorator">${O(r.decorator)}</span>`,n=null;else if(r.number)e+=`<span class="tok-number">${O(r.number)}</span>`,n=null;else if(r.word){const p=r.word;n==="function"?(e+=`<span class="tok-function-def">${O(p)}</span>`,n=null):n==="class"?(e+=`<span class="tok-class-def">${O(p)}</span>`,n=null):Ze.has(p)?(e+=`<span class="tok-keyword">${O(p)}</span>`,p==="def"?n="function":p==="class"?n="class":n=null):et.has(p)?(e+=`<span class="tok-builtin">${O(p)}</span>`,n=null):(e+=O(p),n=null)}a=s.index+s[0].length}return a<t.length&&(e+=O(t.slice(a))),e}function we(t){const e=(t||"").split(`
`).length;return e>=20?"xs":e>=14?"sm":e>=10?"md":"lg"}function tt(t,e){return e.map(a=>nt(a,t)).join(`
`)}function nt(t,e){const a=at[t.type];return a?a(t,e):`<div class="slide"><div class="slide__body">Unknown slide type: ${d(t.type)}</div></div>`}function h(t,e,a){return`data-edit="${t}/${e}/${a}"`}function oe(t,e,a){return`data-edit-list="${t}/${e}/${a}"`}function W(t,e,a="",n=""){return`<div class="slide ${a}" data-slide-type="${d(t.type)}" data-edit-slide="${d(e.id)}/${d(t.id)}" data-unit="${d(e.id)}" data-tag="${d(e.tag||"")}" data-total="${d(e.total||"")}"${n?" "+n:""}>`}const at={section(t,e){return`${W(t,e,"slide--section")}
    ...
    </div>`},title(t,e){return`${W(t,e,"slide--title")}
      <div class="slide__body">
    ...
    </div>`},bullets(t,e){const a=(t.items||[]).map((n,s)=>`<li ${h(e.id,t.id,`items[${s}]`)}>${U(n)}</li>`).join("");return`${W(t,e,"slide--bullets")}
      <div class="slide__body">
        <div class="bullets">
          ...
          <ul class="bullets__list" ${oe(e.id,t.id,"items")}>${a}</ul>
        </div>
      </div>
    </div>`},question(t,e){return`${W(t,e,"slide--question")}
      <div class="slide__body">
        <div class="qslide">
    ...
    </div>`},split(t,e){const a=n=>{const s=t[n]||{title:"",items:[]},r=(s.items||[]).map((p,f)=>`<li ${h(e.id,t.id,`${n}.items[${f}]`)}>${U(p)}</li>`).join("");return`<div class="split__col">
        <h3 ${h(e.id,t.id,`${n}.title`)}>${d(s.title||"")}</h3>
        <ul ${oe(e.id,t.id,`${n}.items`)}>${r}</ul>
      </div>`};return`${W(t,e,"slide--split")}
      <div class="slide__body">
        <div class="split">
    ...
    </div>`},prose(t,e){return`${W(t,e,"slide--prose")}
      <div class="slide__body">
        <div class="prose">
    ...
    </div>`},concept(t,e){const a=t.diagram||"";return`${W(t,e,"slide--concept")}
      <div class="slide__body">
        <div class="concept">
          ${t.eyebrow?`<div class="concept__eyebrow" ${h(e.id,t.id,"eyebrow")}>${d(t.eyebrow)}</div>`:""}
          <h2 class="concept__statement" ${h(e.id,t.id,"statement")}>${U(t.statement||"")}</h2>
          <div class="concept__diagram">${a}</div>
          ${t.caption?`<div class="concept__caption" ${h(e.id,t.id,"caption")}>${U(t.caption)}</div>`:""}
        </div>
      </div>
    </div>`},image(t,e){const a=t.fit==="contain"?"contain":"cover",n=d(t.src||""),s=d(t.alt||"");return`${W(t,e,"slide--image")}
      <div class="slide__body">
        <div class="image-slide image-slide--${a}">
          <img src="${n}" alt="${s}" />
    ...
    </div>`},demo(t,e){const a=d(t.demo||""),n=!!t.code,s=n?ae(t.code):"",r=n?t.code.split(`
`).length:0;return`${W(t,e,`slide--demo${n?" slide--demo-with-code":""}`,`data-demo="${a}"`)}
      <div class="slide__body">
        <div class="demo">
    ...
    </div>`},code(t,e){const a=(t.language||"python").toLowerCase(),n=a==="python"?ae(t.code||""):d(t.code||""),s=we(t.code||"");return`${W(t,e,`slide--code slide--code-${s}`)}
      <div class="slide__body">
        <div class="code-slide">
          ...
          <pre class="code-slide__code language-${d(a)}"><code ${h(e.id,t.id,"code")}>${n}</code></pre>
    ...
    </div>`},"interactive-code"(t,e){const a=t.regions||[],n=t.tooltips||{},s=/^(\s*)#\s*-{3,}\s*.+\s*-{3,}\s*$/,r={};for(const _ of a)for(let v=_.lines[0];v<=_.lines[1];v++)r[v]=_;const p=Object.keys(n).sort((_,v)=>v.length-_.length),x=(t.code||"").split(`
`).map((_,v)=>{const k=v+1,N=r[k];let o=ae(_)||" ";for(const u of p){const C=u.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),b=n[u],z=typeof b=="object"?`data-tip-sig="${d(b.sig||"")}" data-tip-desc="${d(b.desc||"")}" data-tip-url="${d(b.url||"")}"`:`data-tip-sig="" data-tip-desc="${d(b)}" data-tip-url=""`;o=o.replace(new RegExp(C),`<span class="icode-tip" ${z}>${u}</span>`)}const l=N?N.id:"",A=N?N.color:"transparent";return`<div class="${`icode__line${s.test(_)?" icode__line--divider":""}`}" data-region="${l}" data-line="${k}" style="--rc:${A}"><span class="icode__line-num">${k}</span>${o}</div>`}).join(""),R=a.map((_,v)=>{const k=(_.links||[]).map(N=>`<a href="${d(N.url)}" target="_blank" rel="noopener">→ ${d(N.label)}</a>`).join(" ");return`
      ...
      </div>`}).join("");return`${W(t,e,"slide--interactive-code","data-interactive-code")}
      <div class="slide__body">
        <div class="icode">
          ${t.heading?`<h2 class="icode__heading" ${h(e.id,t.id,"heading")}>${d(t.heading)}</h2>`:""}
          <div class="icode__keys-hint">press 1–${a.length} to explore</div>
    ...
    </div>`},"annotated-code"(t,e){const a=ae(t.code||""),n=we(t.code||""),s=(t.annotations||[]).map((r,p)=>`<li ${h(e.id,t.id,`annotations[${p}]`)}>${U(r)}</li>`).join("");return`${W(t,e,`slide--annotated-code slide--acode-${n}`)}
      <div class="slide__body">
        <div class="acode">
          ...
          <div class="acode__cols">
            <pre class="acode__code language-python"><code ${h(e.id,t.id,"code")}>${a}</code></pre>
    ...
    </div>`},live(t,e){const a=(t.items||[]).map((n,s)=>`<li ${h(e.id,t.id,`items[${s}]`)}>${U(n)}</li>`).join("");return`${W(t,e,"slide--live")}
      <div class="slide__body">
        <div class="live">
          <div class="live__badge">● LIVE</div>
          <h2 class="live__heading" ${h(e.id,t.id,"heading")}>${d(t.heading||"")}</h2>
          <ol class="live__list" ${oe(e.id,t.id,"items")}>${a}</ol>
        </div>
      </div>
    </div>`}},ot={id:"01",number:"01",tag:"OPENING THE BOX",total:"12"},it=[{id:"section",type:"section",number:"01",kicker:"Unit",title:"Opening the Box"},{id:"welcome",type:"title",eyebrow:"Game Programmierung  ·  SS26",title:"Welcome.",subtitle:"Twelve weeks to build a real game without an engine. Let's take a look under the hood."},{id:"how-it-works",type:"bullets",heading:"What to expect.",lead:"This is how the course will work:",items:["<b>Bring your laptop</b> — Python, Git, and your codebase. Every session.","<b>Short input, long workshop.</b>","<b>Find a partner / rubberduck.</b>","<b>Four exercises.</b> Mandatory part of your grade.","<b>Ask early.</b>"]},{id:"why-question",type:"question",eyebrow:"Question for the room",question:"Why would anyone write a game without an engine"},{id:"why-answer",type:"image",src:"/img/shoggoth.avif",alt:"The Shoggoth — a lovecraftian mass of eyes and tentacles",caption:"To understand the Shoggoth.",fit:"contain"},{id:"today",type:"demo",eyebrow:"TODAY",heading:"A program where you move a shape when you press a button.",demo:"moving-square",hint:"WASD to move",code:`import pygame
pygame.init()
...
└── Shot
    └── EnemyShot        ← bullets the enemies fire`,caption:"The hierarchy in the skeleton. Each branch is one kind of game thing. For <b>Ü003</b>, you read this code, understand the shape, and add features to existing classes — you don't invent the structure. The skeleton's design is the lesson.",language:"text"},{id:"workshop",type:"bullets",heading:"Workshop — twenty minutes",lead:"Take your Player + Enemy classes from last week. Apply the cure.",items:["<b>Entity base.</b> New class with shared methods: <code>move</code>, <code>take_damage</code>, <code>is_alive</code>.","<b>Inherit.</b> <code>class Player(Entity)</code>, <code>class Enemy(Entity)</code>. Drop the duplicated methods.","<b>super().__init__().</b> Pass parent args up; add child-specific attributes after.","<b>Verify.</b> Game should run identically. If something breaks, the hierarchy is telling you something."]},{id:"homework",type:"bullets",heading:"For next Friday",lead:"Unit 8 is the RealFakeGame workshop. Ü003 (RealFakeGame pt.1) is live.",items:["<b>Workshop pushed.</b> Your refactored Player + Enemy with Entity base.","<b>Read the skeleton.</b> Understand the existing hierarchy before you write code.","<b>Bring your hierarchy questions.</b> Designing class trees is hard. We'll workshop yours in Unit 8."]},{id:"questions",type:"question",eyebrow:"Before you go",question:"Questions"}],Tt={meta:kt,slides:xt},At={id:"08",number:"08",tag:"DEBUGGING",total:"27"},Et=[{id:"section",type:"section",number:"08",kicker:"Unit",title:"Debugging & Exceptions"},{id:"checkin",type:"question",eyebrow:"Check-in",question:"What's the weirdest bug in your RealFakeGame right now?"},{id:"today",type:"bullets",heading:"Today.",lead:"Debugging.",items:["<b>What a bug is</b> — and the method for finding it.","<b>The traceback</b> — how to read it.","<b>Looking inside</b> — print, debug overlay, assert, breakpoint().","<b>Exceptions</b> — raise, try/except, where to catch.","<b>Bonus: unit tests</b> — first contact with pytest.","<b>Workshop</b> — Ü003 build time. Due next week."]},{id:"section-bug",type:"section",number:"08",kicker:"I",title:"What a bug is"},{id:"false-belief",type:"prose",eyebrow:"The core mental model",heading:"A bug is a false belief",body:"The computer is never wrong — it runs exactly what you wrote. A bug means <b>the program in your head</b> and <b>the program in the file</b> disagree. Debugging is finding which of your beliefs is false. The map, not the territory.<br><br>That's why staring rarely works: you re-read the code with the same model that wrote the bug. What breaks the loop is <i>evidence</i> from the running program.",caption:`Kernighan's law: "Debugging is twice as hard as writing the code in the first place." Code written at the limit of your cleverness is beyond your ability to debug.`},{id:"method",type:"code",eyebrow:"The method",heading:"Debugging is the scientific method",code:`1. REPRODUCE    make it happen on demand
                → in a game full of dice: random.seed(42)

2. OBSERVE      what EXACTLY happens? not "it breaks" —
                which frame, which entity, which value?

3. HYPOTHESIZE  one specific false belief:
                "player must be None by the time enemies update"

4. TEST         the cheapest experiment that could prove
                you WRONG:  print(self.player) at line 26

5. FIX          one change. only one.

6. VERIFY       reproduce again — gone?
                and did the fix break anything else?`,caption:"",language:"text"},{id:"bisection",type:"code",eyebrow:"Searching the haystack",heading:"Halve, don't read",code:`# The crash is somewhere in 1,500 lines.
# Do not read 1,500 lines.

def update(self):
    self.update_player()
    # self.update_enemies()      # ← disabled. crash gone?
    # self.update_shots()        #   then the bug lives in
    # self.update_obstacles()    #   this half. halve again.

# 1,500 → 750 → 375 → 188 → 94 → 47 → 24 → 12 → 6 → 3 → 1
# Eleven experiments. Not eleven hundred.`,caption:'Binary search — the same idea as guessing a number between 1 and 1000 in ten questions — applied to <i>causes</i>. log₂(n) beats n every time someone is patient enough to use it. It works on history too: "it still worked on Tuesday" → <code>git bisect</code> halves the commits between Tuesday and now the same way.',language:"python"},{id:"section-traceback",type:"section",number:"08",kicker:"II",title:"The traceback"},{id:"traceback-read",type:"code",eyebrow:"Reading tracebacks",heading:"Read it bottom-up",code:`Traceback (most recent call last):
  File "main.py", line 44, in <module>
    game.run()
  File "main.py", line 31, in run
    self.update()
  File "main.py", line 26, in update
    enemy.update(self.player)
  File "enemy.py", line 18, in update
    if self.rect.colliderect(target.rect):
AttributeError: 'NoneType' object has no attribute 'rect'`,caption:"<b>Last line first</b> — what went wrong: something is <code>None</code> that shouldn't be. <b>Line above it</b> — where: <code>enemy.py:18</code>, and since <code>self.rect</code> would name the enemy, the <code>None</code> is <code>target</code>. <b>Then climb</b>: who passed <code>target</code>? <code>main.py:26</code> passed <code>self.player</code>. So <code>self.player</code> is <code>None</code>. The traceback hands you the whole call chain — your job starts where it ends: <i>who made the None?</i>",language:"text"},{id:"error-vs-fault",type:"code",eyebrow:"Why debugging is hard, in one slide",heading:"The crash site is not the fault site",code:`# main.py — the FAULT. Committed at frame 1:

def restart(self):
    self.player = self.make_player()

def make_player(self):
    p = Player(WIDTH // 2, HEIGHT - 80)
    # ← no return statement. Python silently returns None.


# enemy.py — the CRASH. Frame 7,233, two minutes later:

    if self.rect.colliderect(target.rect):
AttributeError: 'NoneType' object has no attribute 'rect'`,caption:"A function without <code>return</code> returns <code>None</code> — silently. The wrong value is created in one place, stored, carried around for two minutes, and detonates somewhere else entirely. <b>That distance between cause and symptom is the entire difficulty of debugging.</b> Everything in the next two movements — asserts, early raises — is a tool for shrinking it.",language:"python"},{id:"section-inside",type:"section",number:"08",kicker:"III",title:"Looking inside"},{id:"print-well",type:"code",eyebrow:"The oldest tool, used properly",heading:"A print is a probe — it should answer a question",code:`# Noise:
print("here")

# Signal — labeled state, at the boundary of the suspect system:
print(f"[spawn] wave={wave} enemies={len(self.enemies)}")

# Print on CHANGE, not 60×/second:
if player.hp != last_hp:
    print(f"[hit] hp {last_hp} -> {player.hp}")
    last_hp = player.hp

# Or on demand — F1 dumps the world:
if event.key == pygame.K_F1:
    print(f"frame={frame} hp={player.hp} enemies={len(self.enemies)}")`,caption:"Label the probe, print values not greetings, tame the 60-FPS firehose. When the question is answered, delete the probe.",language:"python"},{id:"draw-state",type:"code",eyebrow:"The game programmer's debugger",heading:"Draw your state — the screen beats the console",code:`DEBUG = True                      # settings.py — flip to ship

# at the END of draw(), on top of everything:
if DEBUG:
    for e in self.enemies:
        pygame.draw.rect(screen, (255, 0, 0), e.get_rect(), width=1)
    pygame.draw.rect(screen, (0, 255, 0), self.player.get_rect(), width=1)`,caption:`"Why didn't that shot hit?" is invisible in the console and obvious on screen: <b>the sprite is not the hitbox.</b> Same trick for velocity vectors, spawn zones, AI targets. Keep the flag in all semester.`,language:"python"},{id:"assert",type:"code",eyebrow:"The tripwire",heading:"assert",code:`def take_damage(self, n):
    assert n >= 0, f"negative damage: {n}"
    self.hp = max(0, self.hp - n)

# assert is exactly this, nothing more:
#   if not (n >= 0):
#       raise AssertionError(f"negative damage: {n}")

# True  → nothing happens. zero cost, program continues.
# False → the program stops AT THE FAULT:

Traceback (most recent call last):
  File "player.py", line 31, in take_damage
    assert n >= 0, f"negative damage: {n}"
AssertionError: negative damage: -25`,caption:"Same machinery as every exception — but the crash now points at the fault, not at a symptom 7,000 frames later. That's the whole trade: name your belief, and the program checks it every frame for free.",language:"python"},{id:"breakpoint",type:"code",eyebrow:"Stopping time",heading:"breakpoint()",code:`def update(self):
    if self.player.hp > self.player.max_hp:   # "impossible"
        breakpoint()

# The game freezes mid-frame. Your TERMINAL becomes a prompt:

> main.py(52)update()
-> for enemy in self.enemies:
(Pdb) p self.player.hp
137
(Pdb) p self.player.max_hp
100
(Pdb) n             # run the next line, pause again
(Pdb) c             # continue at full speed`,caption:`<code>breakpoint()</code> drops into pdb, Python's built-in debugger. You see what the paused function sees: <code>self</code>, its locals, the module's globals — <code>p</code> evaluates any expression in that scope, <code>u</code>/<code>d</code> climb the call stack. The window may report "not responding" — it's paused, not dead. VS Code's red dot is the same mechanism with a GUI.`,language:"python"},{id:"pdb-commands",type:"code",eyebrow:"Reference — skim now, read back when stuck",heading:"pdb — the handy ones",code:`ll                        list the whole current function, arrow at the line
w                         where am I — full call stack (pairs with u / d)
a                         print the current function's arguments
s                         step INTO the call on this line (n steps over it)
r                         run until this function returns — shows the value
b enemy.py:18             set a breakpoint from inside pdb, then c
display self.player.hp    auto-print on every stop — watch a value drift
interact                  full Python REPL inside the paused frame
!self.player.hp = 100     force-write state, then c — instant cheat console
q                         quit (kills the program)`,caption:"Two sleepers: <code>display</code> for values that drift over frames, <code>!</code> to rewrite reality mid-pause and watch what happens.",language:"text"},{id:"section-exceptions",type:"section",number:"08",kicker:"IV",title:"Exceptions"},{id:"exception-machinery",type:"code",eyebrow:"What you've been looking at all semester",heading:"An exception is control flow, not an error message",code:`def load_level(path):
    if not path.endswith(".rfg"):
        raise ValueError(f"not a level file: {path}")
    ...

# raise stops the function mid-line and climbs back through
# every caller until someone catches it:
#
#   load_level → start_game → run → <module>
#       ╳ raised ... not caught ... not caught ... nobody home
#
# Nobody catches → Python prints the traceback and exits.`,caption:"You've been reading this machinery's output since week one. And <code>raise</code> is a tool: raising early, near the fault, shrinks the cause→symptom distance on purpose.",language:"python"},{id:"try-except",type:"code",eyebrow:"Catching — and the cardinal sin",heading:"Catch what you expect. Nothing more.",code:`# Expected failure, caught by name — all else still crashes:
try:
    with open("highscore.json") as f:
        data = json.load(f)
except FileNotFoundError:      # first launch, no file yet
    data = {"best": 0}         # the plan

# The bare except catches EVERYTHING. Don't:
try:
    score = compute_score(enemys_killed)  # typo → NameError —
except:                                   #   Python reporting your bug
    score = 0                             #   ...report caught, binned

# No crash, no traceback — the score is just always 0.`,caption:"A bare <code>except:</code> can't tell <i>the failure you planned for</i> from <i>a bug Python is trying to report</i>. It doesn't make code safe — it destroys the evidence and turns a five-second fix into a silent, permanent wrong value.",language:"python"},{id:"lbyl-eafp",type:"code",eyebrow:"Two instincts",heading:"LBYL and EAFP",code:`# LBYL — Look Before You Leap. Check, then act.
# (your C# / Unity reflex:  if (rb != null) ...)
if key in config:
    v = config[key]
else:
    v = DEFAULT
...
# Act, then handle the failure. Python's grain.
try:
    v = config[key]
except KeyError:
    v = DEFAULT`,caption:'Same result, different question. LBYL asks <i>"will this work?"</i> before acting; EAFP just acts and deals with the failure if it comes. Both are legitimate Python — what differs is <b>who notices the failure</b>: your check, or the operation itself.',language:"python"},{id:"which-when",type:"code",eyebrow:"Which, when",heading:"EAFP at the boundary. LBYL in your logic.",code:`# EAFP where the outside world can betray you — you can't
# pre-check every betrayal:
try:
    with open("save.json") as f:
        data = json.load(f)
except FileNotFoundError:        # missing — first launch
    data = DEFAULT_SAVE
except json.JSONDecodeError:     # exists, but corrupted
    data = DEFAULT_SAVE

# LBYL inside your own game logic — "failure" is a normal
# branch, not an emergency:
if player.invulnerable > 0:
    return`,caption:"Files, input, network: the checks can't enumerate every failure (exists-but-corrupted, exists-but-empty), so try it and name the recoveries. Your own logic: plain <code>if</code>s — an exception there is just control flow in a costume. And deepest inside: catch <i>nothing</i>. Let it crash, read the traceback, fix the fault.",language:"python"},{id:"section-tests",type:"section",number:"08",kicker:"V",title:"Bonus: unit tests"},{id:"first-test",type:"code",eyebrow:"A debugging session you only pay for once",heading:"Your first unit test",code:`# test_entities.py
from player import Player

def test_damage_floors_at_zero():
    p = Player(100, 100)
    p.take_damage(9_999)
    assert p.hp == 0              # not -9,899

def test_dead_is_not_alive():
    p = Player(100, 100)
    p.take_damage(p.hp)
...
# $ uv run pytest
# ============ 2 passed in 0.03s ============`,caption:"You've verified <code>take_damage</code> a dozen times by launching the game and walking into an enemy. A unit test is that manual poke <b>written down</b>: pytest finds every <code>test_*.py</code> file, runs every <code>test_*</code> function, and a failed <code>assert</code> is a failed test with a full report. Milliseconds, no window, every time. And the habit that compounds: <b>every bug you fix becomes a test</b> — that bug never gets to come back.",language:"python"},{id:"testable-core",type:"prose",eyebrow:"What tests can and can't do",heading:"Test the core. Play the shell.",body:`Your game has two kinds of code. The <b>logic core</b>: damage math, collision predicates, level parsing, shop prices — functions where input goes in and output comes out, no screen required. And the <b>audiovisual shell</b>: drawing, screenshake, whether the jump feels right — verified by eyes and hands, not by asserts.<br><br>Unit tests are superb at the core and useless at the shell. Nobody unit-tests "does it feel good." Be honest about the boundary — and notice the design pressure it creates: <b>the more logic you keep out of your draw code, the more of your game can be tested.</b> It's the same force that pushed you from one giant file toward classes in Unit 6. Good structure and testability are the same thing wearing different hats.<br><br>Today: write <i>one</i> test for <code>take_damage</code>. We go deeper in Unit 11.`,caption:""},{id:"workshop",type:"bullets",heading:"Workshop — Ü003 build time",lead:"Apply the toolkit to your actual game:",items:["<b>Reproduce first.</b> <code>random.seed(42)</code> — make the bug show up on schedule.","<b>Draw your hitboxes.</b> The <code>DEBUG</code> overlay.","<b>Read the next crash bottom-up.</b> Then hunt where the None was born.","<b>Guard your file I/O.</b> Specific <code>except</code> + default.","<b>One test for <code>take_damage</code>.</b> <code>uv run pytest</code>.","<b>Weirdest bug wins.</b> Gets debugged on the beamer."]},{id:"homework",type:"bullets",heading:"For next Friday",lead:"Ü003 (RealFakeGame pt.1) is due. Next week: state machines & game feel — your game gets a title screen, a death, and juice.",items:["<b>Ü003 final version.</b> All five Punkte — including enemies actually blocked by barriers. If your enemies never touch a barrier, that's a hint about Punkt 3 (they should move), not permission to skip Punkt 2.","<b>Keep the DEBUG overlay.</b> You'll want it every week from now on.","<b>Keep the tests.</b> If you wrote one today, it runs before every submission from here to the final."]},{id:"questions",type:"question",eyebrow:"Before you go",question:"Questions"}],St={meta:At,slides:Et},Pt={id:"09",number:"09",tag:"ARCHITECTURE",total:"34"},It=[{id:"section",type:"section",number:"09",kicker:"Unit",title:"State Machines & Event Systems"},{id:"checkin",type:"question",eyebrow:"Check-in",question:"Are you a fan of spaghetti? No?"},{id:"today",type:"bullets",heading:"Today.",lead:"Architecture. Two patterns for a game that's outgrowing your head.",items:["<b>The real subject</b> — coupling, the thread through the whole term.","<b>State machines</b> — <i>mode</i>: one at a time, and the legal moves between them.","<b>Event systems</b> — <i>communication</i>: how parts signal each other without knowing who's there.","<b>The honest costs</b> — both patterns bite. When not to reach for them.","<b>Together</b> — how they compose, and the one idea underneath all of it.","<b>Workshop</b> — Ü003 due. Then: one refactor that pays off."]},{id:"coupling",type:"prose",eyebrow:"The real subject",heading:"Decoupling",body:"Your check-in answer has a name. A change is scary when too many things are wired to the thing you want to touch — that wiring is <b>coupling</b>, the number of parts that must move when one part moves.<br><br>Look back. Unit 6 split one file into objects so each owns its own data. Unit 7 chose composition to avoid dragging a whole parent class along. Unit 8 hunted bugs across the distance between cause and symptom — that distance <i>is</i> coupling. Today's two patterns attack it head-on.",caption:"Working code is easy. Code that stays easy to change is the entire game — and coupling is what takes 'easy to change' away."},{id:"section-sm",type:"section",number:"09",kicker:"I",title:"State machines"},{id:"boolean-soup",type:"code",eyebrow:"The pain you already have",heading:"Boolean-flag soup",code:`# RealFakeGame, three months in. Flags scattered everywhere:
self.game_over   = False
self.paused      = False
self.in_shop     = False
self.boss_active = False

# 4 booleans -> 16 combinations. How many are legal?
#   game_over=True, in_shop=True      -> ...what?
#   paused=True,    boss_active=True  -> ...?
# You start writing if-guards to forbid combinations
# that should never have been representable in the first place.`,caption:"Every boolean you add doubles the state space, and most of the new room is nonsense. Bugs love nonsense. The fix isn't more guards — it's making the illegal states impossible to write down.",language:"python"},{id:"sm-definition",type:"prose",eyebrow:"The pattern",heading:"A finite state machine",body:"Four things: a fixed set of <b>states</b>, the rule that you are in <b>exactly one</b> at a time, the <b>transitions</b> that are the only legal moves between them, and the <b>events</b> that trigger those moves.<br><br>The 'exactly one' is the whole trick. Sixteen boolean combinations collapse into four named states — <code>title</code>, <code>playing</code>, <code>shop</code>, <code>gameover</code> — and the fifteen nonsense rooms simply stop existing. You can no more be in two states at once than a light switch can be both on and off.",caption:`"Make illegal states unrepresentable." Once you see it, you can't unsee it.`},{id:"sm-dispatch",type:"code",eyebrow:"The simplest real version",heading:"One variable, an if/elif",code:`class Game:
    ...
    def update(self, dt):
        ...
        elif self.state == "gameover": self._update_gameover(dt)`,caption:"This is exactly what the RealFakeGame reference does — a string and an if/elif, no classes, no framework. For four states it is not a compromise; it's the right answer. Architecture is climbing the ladder only as far as the problem makes you.",language:"python"},{id:"sm-transition",type:"code",eyebrow:"The one upgrade worth making early",heading:"Make the transition a door you control",code:`def change_state(self, new):
    assert new in self.STATES, f"unknown state: {new}"   # Unit 8
    print(f"[state] {self.state} -> {new}")              # free log
    self.state = new

# Every state change now runs through ONE method.
#   Typo'd target?  the assert catches it instantly.
#   Mystery jump at 2am?  the log already named
#   who went where, and when.`,caption:"Nearly free, hugely useful. When one method is the only way state changes, it's the one place to validate, to log, and — next slide — to run logic on the way in and out. Scattered <code>self.state = ...</code> assignments can do none of that.",language:"python"},{id:"sm-enter-exit",type:"code",eyebrow:"Where game logic actually lives",heading:"Enter and exit actions",code:`def change_state(self, new):
    self._on_exit(self.state)      # leaving the old state
    self.state = new
    self._on_enter(new)            # arriving in the new one
...
def _on_enter(self, state):
    ...
    if state == "playing":
        self.spawn_wave()`,caption:"A surprising amount of logic isn't 'while in a state' — it's 'the instant you enter or leave one.' Save the score the moment you hit gameover; spawn the wave the moment play begins. The transition is a place where code wants to live.",language:"python"},{id:"sm-state-pattern",type:"prose",eyebrow:"The top rung",heading:"When a state earns its own class",body:"When a state grows past a few lines, promote it from a string to a <b>class</b> — one per state, each with <code>enter()</code>, <code>update(dt)</code>, <code>draw()</code>, <code>exit()</code>. The machine just holds <code>self.current</code> and calls <code>self.current.update(dt)</code>; it no longer knows <i>which</i> state that is.<br><br>That's polymorphism (Unit 7) pointed at time. Adding a state becomes <i>adding a class</i> — the file you touch is new, not the one that already works. The Gang of Four call this the <b>State pattern</b>. The catch: it's more ceremony. For four small states, the elif wins.",caption:"Same ladder, top rung. Climb when the complexity makes you — not before."},{id:"sm-everywhere",type:"prose",eyebrow:"You already use this",heading:"Once you have the lens, they're everywhere",body:"A turnstile (locked ↔ unlocked). A traffic light, the textbook case. Every enemy AI you'll write — <code>idle → chase → attack → flee</code>, with line-of-sight and health as the triggers. UI flows, network connections, the regex engine matching this sentence.<br><br>Your game's title/playing/shop/gameover machine is just the top-level one. Inside <code>playing</code>, the boss runs its own. Inside the boss, each phase. A state can contain a whole machine — a <b>hierarchical</b> state machine, and it's how real games keep the complexity boxed.",caption:"You're not learning a trick. You're learning to recognise a shape that was already there."},{id:"bridge",type:"prose",eyebrow:"The turn",heading:"One mode at a time. Now — who tells whom?",body:"The state machine pinned the <b>mode</b> — which one, and the legal moves to the next.<br><br>But zoom into <code>playing</code> and it's a crowd. An enemy dies — and the score, particles, audio, camera, and combo counter all care. How does the enemy tell them all, <i>without becoming a class that knows about all of them?</i><br><br>Not a question about mode. A question about <b>who is allowed to know about whom</b>.",caption:""},{id:"section-events",type:"section",number:"09",kicker:"II",title:"Event systems"},{id:"junction-box",type:"code",eyebrow:"The pain, made concrete",heading:"Enemy.die() becomes a switchboard",code:`class Enemy:
    def die(self):
        game.score += self.points              # knows Score
        game.particles.burst(self.x, self.y)   # ...and Particles
        game.sound.play("explosion")           # ...and Audio
        game.camera.shake(5)                   # ...and Camera
        game.combo.increment()                 # ...and Combo`,caption:"<code>Enemy.die()</code> now depends on five unrelated systems. Every new reaction — a quest tick, a screen flash, an achievement — edits this method. The enemy has quietly become the switchboard for the whole game, and you can't even test it without booting the camera and the mixer.",language:"python"},{id:"indirection",type:"prose",eyebrow:"The insight",heading:"Another level of indirection",body:"The cure is a middleman. The enemy shouldn't <i>call</i> five systems; it should <b>announce</b> one fact — 'I died' — to the room, then go back to being an enemy. Whoever cares is listening.<br><br>This is the oldest move in the book. David Wheeler: <b>'All problems in computer science can be solved by another level of indirection.'</b> Put something <i>between</i> the thing that happens and the things that react, and you cut every direct wire at once.",caption:"Wheeler's line has a second half we'll need shortly: '…except the problem of too many levels of indirection.' Hold that thought."},{id:"pubsub",type:"prose",eyebrow:"The pattern",heading:"Publish / subscribe",body:"Also called <i>observer</i>, or an <i>event bus</i>. Three roles:<br><br>An <b>event</b> is a named thing that happened, maybe carrying data: <code>enemy_died</code>, with the enemy and its points. A <b>publisher</b> announces events and <i>does not know who is listening</i> — or whether anyone is. A <b>subscriber</b> registers interest in an event type and gets called when it fires.<br><br>The publisher and the subscriber never reference each other. They each reference only the bus in the middle.",caption:""},{id:"the-bus",type:"code",eyebrow:"The whole thing",heading:"An event bus is fifteen lines",code:`class EventBus:
    def __init__(self):
    ...
    def publish(self, event, **data):
        for callback in self._subs.get(event, []):
            callback(**data)`,caption:"That's the entire pattern. The thing with the intimidating name is a dict of lists of functions — and you've had dicts since Unit 5, lists since Unit 2, functions-as-values since your first callback. Nothing here is new except the arrangement.",language:"python"},{id:"bus-usage",type:"code",eyebrow:"Wiring it up",heading:"Subscribe at startup, publish anywhere",code:`bus = EventBus()

# At startup — each system wires itself, in its own file:
bus.subscribe("enemy_died", lambda enemy, points: score.add(points))
bus.subscribe("enemy_died", lambda enemy, points: particles.burst(enemy.x, enemy.y))
bus.subscribe("enemy_died", lambda enemy, points: sound.play("explosion"))

# The enemy now knows the bus, and nothing else:
class Enemy:
    def die(self):
        bus.publish("enemy_died", enemy=self, points=self.points)`,caption:"Five dependencies became one. Adding a screen flash tomorrow is a new <code>subscribe</code> line in the effects file — <code>Enemy</code> never hears about it. The thing that happens and the things that react now grow independently.",language:"python"},{id:"ref-lambda",type:"code",eyebrow:"Reference — skim now, read back later",heading:"lambda — a function with no name",code:`# lambda PARAMS: EXPRESSION  — anonymous function;
# the expression IS the return value (no 'return', no name).

bus.subscribe("enemy_died", lambda enemy, points: score.add(points))

# Exactly the same as naming it first, then passing the name:
def on_enemy_died(enemy, points):
    return score.add(points)
bus.subscribe("enemy_died", on_enemy_died)

# Body is ONE expression — no statements. Need two steps? def.`,caption:"Good for a trivial one-liner. The catch: it's anonymous — a traceback shows only <code>&lt;lambda&gt;</code>, 'find usages' can't see it. For anything you'll debug, a named <code>def</code> earns its keep.",language:"python"},{id:"ref-subscribe",type:"code",eyebrow:"Reference — skim now, read back later",heading:"Under the hood: subscribe",code:`def subscribe(self, event, callback):
    self._subs.setdefault(event, []).append(callback)

# setdefault(key, default): return the value at key,
# inserting default FIRST if the key is missing.
# The long way it replaces:
#   if event not in self._subs:
#       self._subs[event] = []      # empty box, first time only
#   self._subs[event].append(callback)

# [] is an empty list — a box with zero items. append puts
# one IN it:   [] -> [add_score] -> [add_score, burst]
# (never [ , add_score] — there was no blank slot to fill.)`,caption:"<code>setdefault</code> hands back the list (the value), not the key. First time an event is seen it makes the empty box; after that it returns the existing box and throws the spare <code>[]</code> away. <code>append</code> fills the box.",language:"python"},{id:"ref-publish",type:"code",eyebrow:"Reference — skim now, read back later",heading:"Under the hood: publish",code:`def publish(self, event, **data):
    for callback in self._subs.get(event, []):
        callback(**data)

# get(key, default): return the value at key, else the
# default — WITHOUT inserting anything. setdefault's
# read-only twin.

# get("enemy_died", []) -> [add_score, burst]   # the LIST
# get("nobody_home", []) -> []                   # loop 0 times,
#                                                #   silent no-op
# Plain self._subs[event] would raise KeyError instead.`,caption:"<code>get</code> returns the <i>list of callbacks</i> for that event — not the dict, not a single callback. The loop calls each one. The <code>[]</code> default makes 'nobody subscribed' do nothing quietly instead of crashing.",language:"python"},{id:"ref-star-kwargs",type:"code",eyebrow:"Reference — skim now, read back later",heading:"* and ** — pack, unpack, ignore",code:`# Same symbol, mirror jobs depending on WHERE it sits:
def publish(self, event, **data):   # DEF site: PACK kwargs -> dict
    callback(**data)                # CALL site: UNPACK dict -> kwargs

data = {"enemy": goblin, "points": 10}
callback(**data)         #  ==  callback(enemy=goblin, points=10)

#  *  -> POSITIONAL args, packed into a TUPLE
#  ** -> KEYWORD    args, packed into a DICT   (match by NAME)

# A callback takes only what it names; **_ swallows the rest:
def add_score(points, **_):  ...   # points=10 matched; enemy ignored
def play_sound(**_):         ...   # everything ignored ( _ = junk)`,caption:"At a <code>def</code> it collects leftovers; at a call it spreads them back out. That mirror is why <code>publish</code> can forward data it never names. <code>**_</code> is just '<code>**</code> into a throwaway dict called <code>_</code>' — so a subscriber can accept the broadcast and use only the fields it wants.",language:"python"},{id:"ref-carriers",type:"code",eyebrow:"Reference — where this goes next",heading:"Three ways to carry the data",code:`# 1. KEYWORDS (this deck) — clean, self-documenting callbacks
bus.publish("enemy_died", enemy=g, points=10)
def add_score(enemy, points): score.add(points)

# 2. PLAIN DICT — explicit, but dig in by key everywhere
bus.publish("enemy_died", {"enemy": g, "points": 10})
def add_score(data): score.add(data["points"])

# 3. EVENT OBJECT — type-safe, tooling-friendly, recordable
@dataclass
class EnemyDied: enemy: Enemy; points: int
bus.publish(EnemyDied(g, 10))
def add_score(e): score.add(e.points)`,caption:"All three carry the same idea; only the wrapper differs. <b>Keywords</b> read cleanest for a small game. <b>Objects</b> win in large/typed codebases — no magic strings, autocomplete, events you can log or replay. The carrier is an orthogonal choice; the decoupling is identical.",language:"python"},{id:"what-we-bought",type:"prose",eyebrow:"Name the win",heading:"Decoupling, precisely",body:"<b>Enemy</b> no longer depends on score, audio, particles, or camera — only on the bus. You can test <code>die()</code> with a fake bus and assert it published the right event, no window required (Unit 8's testable core). New reactions are new files, not edits to old ones. And the systems don't know <i>each other</i> exists — audio and scoring are strangers who happen to attend the same announcements.<br><br>That's <b>decoupling</b>: the number of things that must change together drops toward one.",caption:"Coupling is the count of things that move when one thing moves. Every pattern this term has been chipping that count down."},{id:"cost-invisible",type:"prose",eyebrow:"The bill — part one",heading:"Control flow goes invisible",body:"With a direct call, 'what happens when an enemy dies?' is answered by Ctrl-clicking <code>die()</code> and reading. With a bus, that question has <b>no answer in the code</b> — it's whoever subscribed, scattered across five files, resolved at runtime.<br><br>The debugging tax is real (Unit 8 again): a traceback through the bus tells you a callback failed, not <i>who registered it or why it fired</i>. 'Who is listening to this event?' becomes a genuine investigation. You traded coupling for fog.",caption:"Direct calls are legible and rigid. Events are flexible and opaque. No free lunch — only a choice of which problem you would rather have."},{id:"cost-ordering",type:"code",eyebrow:"The bill — part two",heading:"Ordering, and ghosts",code:`# ORDERING: subscribers fire in subscription order. If the
# "new highscore!" check runs before score.add(), it reads
# the OLD score. A bug that moves when you reorder imports.

# DANGLING: subscribe a method, the bus holds the reference —
bus.subscribe("enemy_died", self.on_kill)
# ...the enemy dies and leaves the game, but the bus still
# holds self.on_kill. It is never collected, and it still
# fires: a ghost reacting to events from beyond the grave.`,caption:"The fix for ghosts is discipline — whatever subscribes must unsubscribe when it dies. (Python's <code>weakref</code> can automate it; a rabbit hole for another day.) These bugs are quiet, which makes them expensive.",language:"python"},{id:"when-not-to",type:"prose",eyebrow:"The judgment",heading:"When NOT to reach for it",body:"Wheeler's second half comes due: <b>too much indirection is its own disease.</b> When every interaction is an event, control flow dissolves and nothing is legible — 'event soup'. The cure, overdosed, becomes the illness.<br><br>So: <b>direct call</b> when one object genuinely owns or needs another — the player telling <i>its own</i> gun to fire isn't coupling, it's structure. <b>Event</b> when one thing happens and N <i>independent</i> systems react, especially across boundaries: gameplay shouting to audio, vfx, ui, score. The smell is <b>fan-out to strangers</b>.",caption:"Same judgment as inheritance vs composition (Unit 7). The tool is easy; knowing when not to reach for it is the actual skill. If you can't say why a call should be an event, leave it a call."},{id:"section-together",type:"section",number:"09",kicker:"III",title:"Together"},{id:"compose",type:"code",eyebrow:"The two patterns, composed",heading:"The bus drives the state machine",code:`# A subscriber flips the machine — the SM is just a listener:
bus.subscribe("player_died", lambda **_: game.change_state("gameover"))

# enemy hits player -> player's hp runs out -> it announces:
def take_damage(self, n):
    self.hp -= n
    if self.hp <= 0:
        bus.publish("player_died")

# Inside "playing", the same bus decouples every reaction.`,caption:"The state machine is the skeleton — which mode, which transitions. The bus is the nervous system — signals between organs that share no wire. The move into <code>gameover</code> is itself just an event someone subscribed to.",language:"python"},{id:"one-idea",type:"prose",eyebrow:"The point of the whole term",heading:"Four topics, one idea",body:"Step back. Encapsulation (Unit 6), composition (Unit 7), state machines, event buses — they look like four different subjects. They are one.<br><br>Each is a way to <b>shrink how much you must hold in your head at once</b>: a state you can reason about without the other fourteen; a system you can change without touching five others. The enemy of a growing codebase isn't difficulty — it's everything being tangled to everything. These patterns are the untangling.<br><br>That isn't a Python idea or a game idea. It's <i>the</i> idea. The rest is details.",caption:"Write code a future stranger can change in one place. That stranger is usually you, three weeks from now."},{id:"workshop",type:"bullets",heading:"Workshop — one refactor that pays off",lead:"Ü003 is due today — submit, then pick one:",items:["<b>Explicit states.</b> Scattered booleans → one <code>self.state</code> + if/elif.","<b>The good upgrade.</b> One <code>change_state()</code> with an <code>assert</code> + a log.","<b>Cut one switchboard.</b> Tangled method + 15-line bus + one reaction moved out.","<b>Feel it.</b> Add a second reaction — without touching the publisher.","<b>Bring the messy one.</b> Best 'touches too much code' refactored on the beamer."]},{id:"homework",type:"bullets",heading:"For next week",lead:"Next week: level design & progression — and Ü004 (RealFakeGame pt.2) lands. Your game grows; today's patterns are how it grows without collapsing.",items:["<b>Ü003 submitted.</b> All five Punkte.","<b>One pattern, applied.</b> Explicit states or one real event — in your actual game, not a toy.","<b>The change that scared you.</b> Make it. Notice whether today made it smaller."]},{id:"questions",type:"question",eyebrow:"Before you go",question:"Questions"}],Rt={meta:Pt,slides:It},$t={id:"10",number:"10",tag:"THE SHOP",total:"31"},Ct=[{id:"section",type:"section",number:"10",kicker:"Unit",title:"The Shop"},{id:"checkin",type:"question",eyebrow:"Check-in",question:"You'll rebalance your game fifty times before it's fun. Right now — does changing one number cost you one edit, or twenty?"},{id:"thesis",type:"concept",eyebrow:"What today is really about",statement:"Build the machine once. Change the game by editing data.",diagram:`<div class="dg"><div class="dg-cols-2"><div class="dg-card"><div class="dg-card__title">who hears the input?</div><div class="dg-card__rows"><div class="dg-card__row">contexts route it <span class="dg-card__row__note">by what's on screen</span></div><div class="dg-card__row">push / pop screens <span class="dg-card__row__note">menus, shop, dialogs</span></div></div></div><div class="dg-card"><div class="dg-card__title">what does an upgrade do?</div><div class="dg-card__rows"><div class="dg-card__row">data drives it <span class="dg-card__row__note">a table, not code</span></div><div class="dg-card__row">one machine reads it <span class="dg-card__row__note">add content for free</span></div></div></div></div></div>`,caption:"A shop is the excuse. The lesson is how you keep a system soft enough to change at 2am — <b>machine you write once, data you edit forever</b>."},{id:"sec-input",type:"section",number:"10",kicker:"I",title:"Who hears the input?"},{id:"naive-loop",type:"code",language:"python",eyebrow:"The pain you'll hit",heading:"One loop, every screen",code:`def handle_event(self, e):
    if self.state == "playing":
        if e.type == KEYDOWN and e.key == K_SPACE:
            self.player.fire()
    elif self.state == "shop":
        if e.type == MOUSEBUTTONDOWN:
            self.try_buy(e.pos)        # buy a card
    elif self.state == "title":
        if e.type == MOUSEBUTTONDOWN:
            self.start_game()
    elif self.state == "paused":
        ...                            # and on, and on`,caption:"Each new screen bolts another <code>elif</code> onto one function, and a click means three different things depending on a variable up top. This is Unit 9's flag-soup wearing an input costume."},{id:"contextual",type:"concept",eyebrow:"The reframe",statement:"The same click means three things.",diagram:'<div class="dg dg-center"><div class="dg-card" style="min-width:900px;"><div class="dg-card__title">one left-click</div><div class="dg-card__rows"><div class="dg-card__row">in PLAYING <span class="dg-card__row__note">fire the weapon</span></div><div class="dg-card__row">in SHOP <span class="dg-card__row__note">buy the card</span></div><div class="dg-card__row">in PAUSE <span class="dg-card__row__note">ignored</span></div></div></div></div>',caption:"Input has no fixed meaning — only a meaning <i>in context</i>. So the context, not the event loop, should decide what an input does."},{id:"state-owns-input",type:"code",language:"python",eyebrow:"The upgrade",heading:"A state owns its input",code:`# Unit 9: "a state earns its own class." The shop did.
class ShopContext:
...
# The loop stops branching. It just forwards:
for e in pygame.event.get():
    self.current.handle_event(e)       # who's in charge`,caption:"The giant if/elif is gone. Each screen is an object that knows its own input. Adding a screen is adding a class — never editing the loop. The loop got dumb on purpose."},{id:"the-stack",type:"concept",eyebrow:"The smart version",statement:"Stack the screens. The top one gets the input.",diagram:'<div class="dg dg-center"><div class="dg-panel" style="min-width:860px;"><div class="dg-panel__row dg-panel__row--selected">CONFIRM? <span class="dg-panel__row__r">top — gets every event</span></div><div class="dg-panel__sep"></div><div class="dg-panel__row">SHOP <span class="dg-panel__row__r">drawn under the dialog</span></div><div class="dg-panel__row">PLAYING <span class="dg-panel__row__r">frozen, still drawn</span></div></div></div>',caption:"Open the shop → push it. Open a confirm dialog → push that. Input always goes to the top; pop to step back. Pause, menus, dialogs — all the same move."},{id:"stack-code",type:"code",language:"python",eyebrow:"The whole thing",heading:"A context stack is a dozen lines",code:`class Game:
    def __init__(self):
        self.stack = [PlayingContext(self)]   # bottom

    def push(self, ctx): self.stack.append(ctx)
    def pop(self):       self.stack.pop()

    def handle_event(self, e):
        self.stack[-1].handle_event(e)    # top only

    def draw(self, screen):
        for ctx in self.stack:            # bottom-up
            ctx.draw(screen)              # shop over game`,caption:"Twelve lines buy every menu you'll ever need. The shop draws over a frozen battlefield; a dialog draws over the shop. Push to enter, pop to leave — the back button writes itself."},{id:"actions-not-keys",type:"concept",eyebrow:"One more, if you're hungry",statement:"Bind keys to intentions, not the other way round.",diagram:'<div class="dg dg-center"><div class="dg-card" style="min-width:980px;"><div class="dg-card__title">raw input → named action</div><div class="dg-card__rows"><div class="dg-card__row">SPACE · A-button · click <span class="dg-card__row__note">&rarr; "confirm"</span></div><div class="dg-card__row">ESC · B-button <span class="dg-card__row__note">&rarr; "cancel"</span></div><div class="dg-card__row dg-card__row--accent">your UI asks: did they confirm? <span class="dg-card__row__note">not: was it space?</span></div></div></div></div>',caption:"Map raw keys to named actions in one table, and your shop asks <i>&ldquo;did they confirm?&rdquo;</i> Rebinding, controllers, and &lsquo;why is this the jump key&rsquo; all become data, not a rewrite."},{id:"sec-upgrade",type:"section",number:"10",kicker:"II",title:"What does an upgrade do?"},{id:"naive-shop",type:"code",language:"python",eyebrow:"The pain, again",heading:"Every upgrade, hand-wired three times",code:`def draw_shop(self):
    self.button("Damage +1", x=100)
    self.button("Health +10", x=300)
    self.button("Speed +1", x=500)     # 1. by hand

def try_buy(self, label):
    if label == "Damage +1":
        self.player.dmg += 1           # 2. by hand
    elif label == "Health +10":
        self.player.hp_max += 10
    elif label == "Speed +1":
        self.player.speed += 1         # 3. priced apart`,caption:"Three upgrades, three places to edit, three chances to forget one. A fourth is surgery in every method. Now imagine forty — and balancing them by hunting magic numbers across the file."},{id:"upgrade-data",type:"code",language:"python",eyebrow:"The upgrade",heading:"An upgrade is a row of data",code:`# One upgrade = one dict.
# The whole shop is just a list of them:
UPGRADES = [
  {"name": "Blades", "rarity": "common", "cost": 50,
   "effect": {"stat": "dmg", "op": "add", "value": 1}},
  {"name": "Hide", "rarity": "common", "cost": 60,
   "effect": {"stat": "hp_max", "op": "add", "value": 10}},
  {"name": "Berserk", "rarity": "rare", "cost": 120,
   "effect": {"stat": "dmg", "op": "mul", "value": 1.5}},
]`,caption:"No code — just data. Adding an upgrade is adding a line. You (at 2am) can balance the entire game by editing numbers, and never touch the machinery that reads them."},{id:"effect-choices",type:"concept",eyebrow:"The real design decision",statement:"Where does the effect live?",diagram:`<div class="dg dg-center"><div class="dg-card" style="min-width:1180px;"><div class="dg-card__title">three ways to store "what it does"</div><div class="dg-card__rows"><div class="dg-card__row">a function in the data <span class="dg-card__row__note">flexible — but can't save or print it</span></div><div class="dg-card__row dg-card__row--accent">a modifier as data &nbsp;{stat, op, value} <span class="dg-card__row__note">tunable, serialisable — the default</span></div><div class="dg-card__row">a subclass per upgrade <span class="dg-card__row__note">pure OOP — a class for every +1</span></div></div></div></div>`,caption:"Most upgrades are &lsquo;change a stat by N&rsquo; — pure data. Express those as data and you get save-files, tooltips, and balance tables for free. Keep code for the genuinely weird ones."},{id:"statbag-apply",type:"code",language:"python",eyebrow:"The machine",heading:"A stat-bag, and one apply()",code:`class Player:
    def __init__(self):
        self.stats = {              # one addressable bag
            "dmg": 1, "hp_max": 100,
            "speed": 5, "crit": 0.0,
        }

def apply_effect(player, effect):   # every upgrade
    stat = effect["stat"]
    op, value = effect["op"], effect["value"]
    if op == "add": player.stats[stat] += value
    if op == "mul": player.stats[stat] *= value`,caption:"Because the stats live in a dict, one tiny function applies <i>any</i> modifier — present or future. Add a stat (&lsquo;lifesteal&rsquo;) and the machine already knows how to buff it. Generic machine, specific data."},{id:"base-vs-current",type:"concept",eyebrow:"A subtlety worth naming",statement:"Permanent bump, or recompute every frame?",diagram:`<div class="dg"><div class="dg-cols-2"><div class="dg-card"><div class="dg-card__title">apply on pickup</div><div class="dg-card__rows"><div class="dg-card__row">mutate the stat once</div><div class="dg-card__row">simplest thing that works <span class="dg-card__row__note">permanent</span></div><div class="dg-card__row">can't un-apply <span class="dg-card__row__note">&rarr; roguelike upgrades</span></div></div></div><div class="dg-card"><div class="dg-card__title">base + modifiers</div><div class="dg-card__rows"><div class="dg-card__row">keep base, sum mods each frame</div><div class="dg-card__row">stackable, removable <span class="dg-card__row__note">shows 10 (+3)</span></div><div class="dg-card__row">more machinery <span class="dg-card__row__note">&rarr; temporary buffs</span></div></div></div></div></div>`,caption:"Permanent roguelike upgrades? Mutate on pickup. Buffs that wear off? Keep a base and recompute. Pick the one your game needs — don't build the hard one for free."},{id:"escape-hatch",type:"code",language:"python",eyebrow:"The 10% that bend the rules",heading:"Leave a clean escape hatch",code:`# 90% are modifiers. The weird ones get a function:
UPGRADES.append(
  {"name": "Ricochet", "rarity": "rare", "cost": 150,
   "effect": {"fn": "enable_ricochet"}})

SPECIAL = {                             # the weird zoo
    "enable_ricochet": lambda p: p.flags.add("ric"),
}

def apply_effect(player, effect):
    if "fn" in effect:                  # special?
        SPECIAL[effect["fn"]](player); return
    ...                                 # else: data path`,caption:"Don't contort the data model for the one upgrade that breaks a rule. Ninety percent stays clean data; the strange ten percent stays honest code — and they don't infect each other."},{id:"sec-draft",type:"section",number:"10",kicker:"III",title:"Pick one of three"},{id:"weighted",type:"concept",eyebrow:"Why uniform random is wrong",statement:"Rarer cards, seen rarer.",diagram:'<div class="dg dg-center"><div class="dg-col" style="gap:var(--s-3); font-family:var(--font-mono); font-size:var(--fs-md);"><div class="dg-row"><span style="display:inline-block; width:300px;">common</span><span class="dg-accent">&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;</span><span class="dg-dim">&nbsp;10</span></div><div class="dg-row"><span style="display:inline-block; width:300px;">rare</span><span class="dg-accent">&#9608;&#9608;&#9608;</span><span class="dg-dim">&nbsp;3</span></div><div class="dg-row"><span style="display:inline-block; width:300px;">legendary</span><span class="dg-accent">&#9608;</span><span class="dg-dim">&nbsp;1</span></div></div></div>',caption:"Uniform random makes legendaries as common as junk, and the meta-game collapses. Give each rarity a weight; the weights are data, so the whole game's feel is a few numbers you tune."},{id:"draft-code",type:"code",language:"python",eyebrow:"The implementation nuance",heading:"Draw three, no duplicates",code:`import random
...
def draft(pool, k=3):
    ...
    picks = []
    for _ in range(k):
        weights = [RARITY[u["rarity"]] for u in pool]
        chosen = random.choices(pool, weights)[0]
        pool.remove(chosen)             # no dupes
        picks.append(chosen)
    return picks`,caption:"<code>random.choices</code> is weighted but draws <i>with</i> replacement — you'd show the same card twice. Draw one, remove it, repeat. Three lines past the obvious, and the draft feels right instead of buggy."},{id:"curation",type:"concept",eyebrow:"The skill is subtraction",statement:"The craft is what you DON'T offer.",diagram:`<div class="dg dg-center"><div class="dg-card" style="min-width:1020px;"><div class="dg-card__title">filter the pool before you draw</div><div class="dg-card__rows"><div class="dg-card__row">drop maxed-out upgrades <span class="dg-card__row__note">no dead picks</span></div><div class="dg-card__row">hide what's useless to this build <span class="dg-card__row__note">no traps</span></div><div class="dg-card__row">guarantee variety <span class="dg-card__row__note">not three of one stat</span></div></div></div></div>`,caption:"&lsquo;Pick one of three&rsquo; is only interesting if all three could plausibly tempt you. A good draft filters first, then draws."},{id:"sec-math",type:"section",number:"10",kicker:"IV",title:"Mathing the curve"},{id:"curves",type:"code",language:"python",eyebrow:"Difficulty is a formula",heading:"Two constants, the whole campaign",code:`# settings.py — every difficulty knob in ONE place
BASE_HP   = 20
HP_GROWTH = 1.35        # per level

def enemy_hp(level):
    # exponential — runs away if you're careless
    return BASE_HP * (HP_GROWTH ** level)

def boss_hp(level):
    # quadratic — bosses spike late
    return BASE_HP * 8 * (level ** 2)`,caption:"Linear feels flat; exponential runs away; quadratic spikes. Whichever you pick, it's <i>one formula and two constants</i> — change <code>HP_GROWTH</code> and the entire campaign re-tunes. Scattered magic numbers can't."},{id:"economy",type:"concept",eyebrow:"The loop you're balancing",statement:"Power and difficulty climb together.",diagram:'<div class="dg dg-center"><div class="dg-col dg-center" style="gap:var(--s-4);"><div class="dg-flow" style="font-size:var(--fs-md); flex-wrap:wrap; justify-content:center;"><span>survive</span><span class="dg-flow__arrow">&rarr;</span><span>score</span><span class="dg-flow__arrow">&rarr;</span><span class="dg-accent">currency</span><span class="dg-flow__arrow">&rarr;</span><span>upgrades</span><span class="dg-flow__arrow">&rarr;</span><span class="dg-accent">power</span></div><div class="dg-label">and every level the enemy curve rises to meet that power &nbsp;&#8635;</div></div></div>',caption:"If power outpaces difficulty the game goes limp; if difficulty wins it's a wall. You tune the two curves against each other — fast, because both are constants in one file."},{id:"sec-change",type:"section",number:"10",kicker:"V",title:"Design for change"},{id:"four-habits",type:"concept",eyebrow:"The actual lesson",statement:"Build so that change is cheap.",diagram:'<div class="dg dg-center"><div class="dg-card" style="min-width:1200px;"><div class="dg-card__title">four habits</div><div class="dg-card__rows"><div class="dg-card__row">data apart from logic <span class="dg-card__row__note">machine once, content forever</span></div><div class="dg-card__row">one source of truth <span class="dg-card__row__note">every tunable in one table</span></div><div class="dg-card__row">make the common change a one-liner <span class="dg-card__row__note">optimise the edit you repeat</span></div><div class="dg-card__row">spec the data shape first <span class="dg-card__row__note">the schema is the contract — Unit 5</span></div></div></div></div>',caption:"None of this is about the shop. It's how you keep <i>any</i> system soft enough to change the night before the deadline — which is when games actually get good."},{id:"payoff",type:"code",language:"python",eyebrow:"What you bought",heading:"Three playtest changes, three small diffs",code:`# Add a whole new upgrade — one row:
UPGRADES.append(
  {"name": "Vampirism", "rarity": "rare", "cost": 140,
   "effect": {"stat": "lifesteal", "op": "add",
              "value": 0.1}})

# Rebalance the back half — one number:
HP_GROWTH = 1.28          # was 1.35

# Add a new stat the shop can already buff:
self.stats["lifesteal"] = 0.0   # no apply change`,caption:"A new item, a difficulty nerf, a new mechanic — each is a line or two in a known place. That's not luck. You bought it with every &lsquo;data, not code&rsquo; decision upstream."},{id:"architecture",type:"concept",eyebrow:"Put it together",statement:"One picture, all of it.",diagram:'<div class="dg dg-center"><div class="dg-col dg-center" style="gap:var(--s-2); font-family:var(--font-mono); font-size:var(--fs-base); line-height:1.2;"><div>input <span class="dg-dim">&rarr;</span> <span class="dg-card" style="min-width:0; padding:6px 18px; display:inline-block;">context stack</span> <span class="dg-dim">&rarr;</span> ShopContext</div><div class="dg-flow__arrow">&darr;</div><div><span class="dg-accent">UPGRADES</span> <span class="dg-dim">(data)</span> <span class="dg-dim">&rarr;</span> draft(weighted) <span class="dg-dim">&rarr;</span> 3 cards</div><div class="dg-flow__arrow">&darr;</div><div>click <span class="dg-dim">&rarr;</span> apply_effect(<span class="dg-accent">effect</span>) <span class="dg-dim">&rarr;</span> player.stats</div><div class="dg-flow__arrow">&darr;</div><div><span class="dg-accent">curve constants</span> <span class="dg-dim">&rarr;</span> next level</div></div></div>',caption:"Every <b>box</b> is a machine you wrote once. Every <b>arrow</b> carries data you'll edit forever. Find the seam between them and your game stays soft."},{id:"workshop",type:"live",heading:"Workshop — build your shop",items:["<b>Promote the shop to a context</b> — give it <code>handle_event</code> and <code>draw</code>.","<b>Make a stack</b> — push the shop, pop on Continue. The battlefield freezes under it.","<b>Write UPGRADES</b> as a list of dicts — five to start, all stat modifiers.","<b>One apply_effect(player, effect)</b>, one <b>draft(pool, 3)</b> with rarity weights.","<b>Move every balance number</b> — costs, growth — into <code>settings.py</code>.","<b>Add one juicy thing</b> — an eased bar, a pop-in, or vary() on every hit. Feel the difference."]},{id:"homework",type:"bullets",heading:"Ü004 — RealFakeGame pt. 2",lead:"The shop, the draft, scaling levels and a boss. Due Unit 12 — bring it building.",items:["<b>A working shop</b> between levels — pick one of three, buy, continue.","<b>Upgrades as data</b> — at least six, all through one apply_effect().","<b>A scaling curve</b> — enemies and prices grow from constants you can show me.","<b>One boss</b> with a stat spike and a telegraphed attack.","<b>One feel pass</b> — damage pop, eased bar, or vary() on hits. Lerp, ease, or vary — your pick."]},{id:"questions",type:"question",eyebrow:"Before you go",question:"Questions"}],Lt={meta:$t,slides:Ct},Nt={id:"11",number:"11",tag:"FEEL & DOCS",total:"35"},Ot=[{id:"section",type:"section",number:"11",kicker:"Unit",title:"Juice & the Paper Trail"},{id:"checkin",type:"question",eyebrow:"Check-in",question:"Two games. Same mechanics, same numbers. One feels crunchy, one feels like a spreadsheet. Where does the difference live?"},{id:"thesis",type:"concept",eyebrow:"What today is really about",statement:"Make it feel good. Then write it down.",diagram:'<div class="dg"><div class="dg-cols-2"><div class="dg-card"><div class="dg-card__title">feel (part I)</div><div class="dg-card__rows"><div class="dg-card__row">three tiny functions <span class="dg-card__row__note">lerp &middot; ease &middot; vary</span></div><div class="dg-card__row">twenty uses each <span class="dg-card__row__note">seasoning, not systems</span></div></div></div><div class="dg-card"><div class="dg-card__title">paper (part II)</div><div class="dg-card__rows"><div class="dg-card__row">three documents <span class="dg-card__row__note">GDD &middot; spec &middot; code docs</span></div><div class="dg-card__row">three readers <span class="dg-card__row__note">designer &middot; builder &middot; the next dev</span></div></div></div></div></div>',caption:"&Uuml;004 is building and last week's machine works. What's left separates a project from a game: it <b>feels</b> right, and someone else — including future-you — can <b>pick it up</b>. Both are far cheaper than they look."},{id:"sec-feel",type:"section",number:"11",kicker:"I",title:"Feel, implemented"},{id:"feel-framing",type:"concept",eyebrow:"Left on the table last week",statement:"Three two-line functions. Twenty uses each.",diagram:'<div class="dg dg-center"><div class="dg-card" style="min-width:1100px;"><div class="dg-card__title">the feel toolkit</div><div class="dg-card__rows"><div class="dg-card__row">lerp(a, b, t) <span class="dg-card__row__note">all smooth motion</span></div><div class="dg-card__row">ease(t) <span class="dg-card__row__note">the curve that makes it feel right</span></div><div class="dg-card__row">vary(v) <span class="dg-card__row__note">the wobble that makes it alive</span></div><div class="dg-card__row dg-card__row--accent">seasoning, not systems <span class="dg-card__row__note">no new mechanics</span></div></div></div></div>',caption:"Unit 9 met game feel as an idea; Unit 10 ran out of clock before this part. It's the design-for-change move one more time: a tiny machine, reused everywhere, tuned by a constant."},{id:"lerp",type:"code",language:"python",eyebrow:"The one function under all of it",heading:"lerp — walk from here to there",code:`def lerp(a, b, t):        # t walks 0.0 -> 1.0
    return a + (b - a) * t

# one line beneath all smooth motion.
# drive t with a timer, 0 up to 1:
x  = lerp(start_x, end_x, t)   # slide a menu in
a  = lerp(255, 0, t)           # fade out
sz = lerp(0, 1, t)             # grow a pop-in
col = lerp(200, 60, t)         # one colour channel
hp = lerp(hp, real_hp, 0.2)    # bar chases value`,caption:"<code>t</code> is just &lsquo;how far along, 0 to 1&rsquo; — drive it with a timer. Position, alpha, size, a colour channel: if it glides, it's this line. Learn it once and half of &lsquo;juice&rsquo; is already yours."},{id:"easing-map",type:"concept",eyebrow:"Why linear feels cheap",statement:"The curve is the feel.",diagram:'<div class="dg dg-center"><div class="dg-card" style="min-width:1120px;"><div class="dg-card__title">bend t before it enters lerp</div><div class="dg-card__rows"><div class="dg-card__row">ease-out <span class="dg-card__row__note">things arriving — menus, pickups, a bar settling</span></div><div class="dg-card__row">ease-in <span class="dg-card__row__note">things leaving — fades, exits</span></div><div class="dg-card__row dg-card__row--accent">ease-out-back <span class="dg-card__row__note">the pop — overshoot, then settle</span></div></div></div></div>',caption:"You don't change <i>what</i> moves — you reshape <i>how</i>. Linear is a robot arm; ease-out is a thing with weight coming to rest. The overshoot of ease-out-back is what reads as &lsquo;juicy&rsquo;."},{id:"easing-code",type:"code",language:"python",eyebrow:"The curves",heading:"Easing is a function on t",code:`def ease_out(t):  return 1 - (1-t)**2   # settle
def ease_in(t):   return t * t          # accelerate

def ease_out_back(t):        # overshoot, then settle
    s = 1.70158
    t -= 1
    return 1 + (s+1) * t**3 + s * t**2

# same lerp, eased t:
sz = lerp(0, 1, ease_out_back(t))   # juicy pop-in`,caption:"Same <code>lerp</code>, eased <code>t</code>. Swapping <code>ease_out</code> for <code>ease_out_back</code> turns a boring grow into a pop that overshoots and bounces back — one function call, completely different feel."},{id:"spring",type:"code",language:"python",eyebrow:"lerp toward a moving target",heading:"approach — catch up with weight",code:`# lerp toward a target that keeps moving:
def approach(cur, target, k=0.2):
    return cur + (target - cur) * k

# one k tunes how heavy it feels:
cam_x = approach(cam_x, player.x)        # camera
hp_shown = approach(hp_shown, hp, 0.1)   # health bar
coins_d = approach(coins_d, coins)       # counter
aim = approach(aim, mouse_angle, 0.3)    # smooth aim
# (scale k by dt if your FPS wanders)`,caption:"The target moves and you never quite arrive — that lag <i>is</i> weight. Camera follow, the health bar easing down, the coin counter rolling up, smooth aim: all this one line, tuned by a single <code>k</code>."},{id:"variance-map",type:"concept",eyebrow:"The 0.9 - 1.1 trick",statement:"Identical is dead.",diagram:'<div class="dg dg-center"><div class="dg-card" style="min-width:1120px;"><div class="dg-card__title">v *= random.uniform(0.9, 1.1) &nbsp;on …</div><div class="dg-card__rows"><div class="dg-card__row">damage numbers <span class="dg-card__row__note">no two hits identical</span></div><div class="dg-card__row">enemy speed &amp; spawn timing <span class="dg-card__row__note">ragged, not metronomic</span></div><div class="dg-card__row">particle size · velocity · lifetime <span class="dg-card__row__note">organic bursts</span></div><div class="dg-card__row">sound pitch &amp; volume <span class="dg-card__row__note">kills the machine-gun sample</span></div><div class="dg-card__row dg-card__row--accent">the range is the knob <span class="dg-card__row__note">0.9-1.1 a whisper · 0.7-1.3 chaos</span></div></div></div></div>',caption:"A game stamps out identical copies; reality never does. One random multiplier, almost anywhere, and the machine-stamp dissolves into something that feels hand-made."},{id:"variance-code",type:"code",language:"python",eyebrow:"One helper, everywhere",heading:"vary — break the uniformity",code:`import random

def vary(v, amount=0.1):     # +/- 10% by default
    lo, hi = 1 - amount, 1 + amount
    return v * random.uniform(lo, hi)

dmg = vary(base_dmg)           # no two hits alike
p.life = vary(0.5, 0.3)        # ragged particles
sfx.set_volume(vary(0.8))      # breathing audio
delay = vary(2.0)              # uneven spawn waves`,caption:"<code>amount</code> is a feel dial: <code>0.1</code> is a subtle shimmer, <code>0.3</code> is wild. Sprinkle it on the numbers a player feels but never reads — and the whole game stops sounding like a photocopier."},{id:"hit-pause",type:"code",language:"python",eyebrow:"The cheapest impact in games",heading:"Hit-pause — freeze on contact",code:`# Freeze the whole game for a few frames on a hit.
hit_stop = 0                   # frames left frozen

def on_big_hit():
    global hit_stop
    hit_stop = 4               # ~0.07s at 60 fps
...
def update(dt):
    ...
    ...                        # else: normal game step`,caption:"A four-frame freeze on impact sells a hit harder than any particle — the brain reads the stutter as force. Stack it with a flash and a shake and a weak punch lands like a truck."},{id:"impact-stack",type:"code",language:"python",eyebrow:"Every big hit, all three layers",heading:"The impact stack",code:`shake = 0.0                    # screen offset, px

def on_big_hit():
    global shake, hit_stop
    shake, hit_stop = 8.0, 4   # shake + freeze

def update():
    global shake
    shake *= 0.9               # exponential decay
def draw(screen, world):       # world = the frame
    dx = random.uniform(-shake, shake)
    dy = random.uniform(-shake, shake)
    screen.blit(world, (dx, dy))`,caption:"Draw the world to an offscreen surface, then blit it shifted — the whole frame kicks. Add a two-frame white flash on top and a weak hit lands like a truck. Three constants (<code>8.0</code>, <code>0.9</code>, <code>4</code>), each one a feel dial."},{id:"damage-numbers",type:"code",language:"python",eyebrow:"The helpers composed — pop, float, fade",heading:"Damage numbers",code:`class DamageNumber:
    def __init__(self, x, y, dmg):
        self.x, self.y0 = x, y
        self.dmg = int(vary(dmg, 0.15))
        self.t = 0.0            # its own tiny clock

    def step(self):
        self.t = min(1.0, self.t + 1/40)
        e = ease_out(self.t)
        self.y = self.y0 - 34 * e          # drift up
        self.alpha = lerp(255, 0, self.t)  # fade
        self.scale = lerp(1.4, 1.0, e)     # pop`,caption:"Every helper from the last ten minutes in one class: <code>vary()</code> so no two hits read alike, <code>ease_out</code> to leap then drift, <code>lerp</code> for fade and pop. Spawn on every hit; delete at <code>t == 1</code>."},{id:"cards-deal",type:"code",language:"python",eyebrow:"Last week's shop, this week's feel",heading:"Deal the draft cards in",code:`# shop.open(): give each card a delayed clock
for i, card in enumerate(self.cards):
    card.t = -i * 0.15      # later cards wait

# each frame, in shop.step():
for card in self.cards:
    card.t = min(1.0, card.t + 1/25)
    e = ease_out_back(max(0.0, card.t))
    card.x = lerp(-CARD_W, card.home_x, e)`,caption:"A negative <code>t</code> is a free stagger — the card simply waits until its clock crosses zero. <code>ease_out_back</code> makes each card overshoot its slot and settle. The Unit 10 shop suddenly feels like a card game."},{id:"feel-payoff",type:"concept",eyebrow:"Where to spend one hour",statement:"An hour of feel, mapped.",diagram:'<div class="dg dg-center"><div class="dg-card" style="min-width:1240px;"><div class="dg-card__title">add … → reach for …</div><div class="dg-card__rows"><div class="dg-card__row">damage numbers pop &amp; float <span class="dg-card__row__note">lerp + ease-out + fade</span></div><div class="dg-card__row">health bar &amp; coins glide <span class="dg-card__row__note">approach()</span></div><div class="dg-card__row">enemy spawn pop-in <span class="dg-card__row__note">scale 0&rarr;1 via ease-out-back</span></div><div class="dg-card__row dg-card__row--accent">every hit: flash + shake + freeze <span class="dg-card__row__note">vary() + hit-pause</span></div><div class="dg-card__row">every SFX &amp; particle wobbles <span class="dg-card__row__note">vary()</span></div></div></div></div>',caption:"None of it is a new system — it's seasoning the ones you have. Line for line, this is the biggest feel-per-effort in the whole codebase. Do it last, after the game works."},{id:"sec-paper",type:"section",number:"11",kicker:"II",title:"The paper trail"},{id:"why-docs",type:"question",eyebrow:"Week 12, 2 a.m.",question:"The boss has 340 HP. Why 340? Nobody remembers. Something should."},{id:"doc-chain",type:"concept",eyebrow:"The same bug, twice",statement:"Docs turn archaeology into a lookup.",diagram:`<div class="dg"><div class="dg-cols-2"><div class="dg-card"><div class="dg-card__title">week 12, no trail</div><div class="dg-card__rows"><div class="dg-card__row">playtest: the boss is trivial <span class="dg-card__row__note">why is boss HP 340? nobody knows</span></div><div class="dg-card__row">afraid to touch it <span class="dg-card__row__note">what else was balanced against it?</span></div><div class="dg-card__row">two hours of re-tuning <span class="dg-card__row__note">and two new bugs</span></div></div></div><div class="dg-card"><div class="dg-card__title">week 12, with the trail</div><div class="dg-card__rows"><div class="dg-card__row">devlog, 21 May <span class="dg-card__row__note">&lsquo;boss 260&rarr;340 — trivial after multishot buff&rsquo;</span></div><div class="dg-card__row">the why is right there <span class="dg-card__row__note">multishot got capped since. that's it</span></div><div class="dg-card__row">settings.py, one edit <span class="dg-card__row__note">thirty seconds, confidence intact</span></div></div></div></div></div>`,caption:"Same codebase, same bug, different past. The difference is a one-line log entry written seven weeks earlier by someone who couldn't know it would matter. That's what documentation buys: your own decisions, back when you need them."},{id:"three-docs",type:"concept",eyebrow:"Not one document — three",statement:"Three documents, three readers.",diagram:'<div class="dg dg-center"><div class="dg-card" style="min-width:1150px;"><div class="dg-card__title">each answers one question</div><div class="dg-card__rows"><div class="dg-card__row">GDD <span class="dg-card__row__note">the designer asks: what game is this?</span></div><div class="dg-card__row">spec <span class="dg-card__row__note">the builder asks: what exactly do I build?</span></div><div class="dg-card__row">code docs <span class="dg-card__row__note">the next dev asks: how do I build ON it?</span></div><div class="dg-card__row dg-card__row--accent">none is written for a grader <span class="dg-card__row__note">each has a reader who actually needs it</span></div></div></div></div>',caption:"&lsquo;Documentation&rsquo; fails when it's one grey blob written for a grader. Split it by reader and each piece gets short, concrete and actually useful — including to future-you, who is also &lsquo;the next dev&rsquo;."},{id:"scaling-law",type:"concept",eyebrow:"How much is enough?",statement:"Effort follows readers &times; lifetime.",diagram:'<div class="dg dg-center"><div class="dg-card" style="min-width:1240px;"><div class="dg-card__title">the same practices, turned up</div><div class="dg-card__rows"><div class="dg-card__row">a weekend jam <span class="dg-card__row__note">good names, constants in one file — the code IS the doc</span></div><div class="dg-card__row">this course <span class="dg-card__row__note">+ a dated one-pager, a README map, comments at the traps</span></div><div class="dg-card__row">others build on it <span class="dg-card__row__note">+ docstrings on the seams, a legend on the knobs</span></div><div class="dg-card__row">a studio, years <span class="dg-card__row__note">+ specs, decision records, style guides, onboarding</span></div><div class="dg-card__row dg-card__row--accent">under-doc: you pay in archaeology <span class="dg-card__row__note">over-doc: the docs rot into lies</span></div></div></div></div>',caption:"One reader — you, today — barely justifies writing anything; every added reader and month multiplies the payoff. There is no &lsquo;proper&rsquo; level. Ask who will read this, and when — then write exactly that much."},{id:"gdd-anatomy",type:"concept",eyebrow:"Writing one that gets read",statement:"A GDD is a decision record.",diagram:`<div class="dg dg-center"><div class="dg-card" style="min-width:1200px;"><div class="dg-card__title">one shape it can take</div><div class="dg-card__rows"><div class="dg-card__row">fantasy <span class="dg-card__row__note">one sentence: who am I, what's the dream?</span></div><div class="dg-card__row">core loop <span class="dg-card__row__note">the verb chain a session cycles through</span></div><div class="dg-card__row">rules that matter <span class="dg-card__row__note">win, lose, and the numbers that carry them</span></div><div class="dg-card__row">feel targets <span class="dg-card__row__note">what should hitting / buying / dying feel like?</span></div><div class="dg-card__row dg-card__row--accent">the scope fence <span class="dg-card__row__note">what this game is NOT — the load-bearing box</span></div></div></div></div>`,caption:"The format is not the point — the test is whether a stranger could rebuild your <i>decisions</i> from it. This shape fits a small arcade game; yours will grow different boxes. Only the NOT-list travels everywhere: every cut written down is an argument you don't have at 2 a.m."},{id:"gdd-example",type:"code",language:"python",eyebrow:"RealFakeGame in one page",heading:"A GDD you'd actually read",code:`# RealFakeGame - one page, v3 (2026-07-02)

Fantasy   lone ship vs. an endless armada
Loop      dodge > shoot > collect > SHOP >
          harder wave
Win/Lose  clear level 5  /  HP hits 0

Feel      hits are crunchy (shake + freeze);
          the shop is a breath between waves

NOT in v1: story, saves, a second weapon,
multiplayer, boss phases. (Load-bearing.)`,caption:"Half a screen, versioned and dated — because it <i>will</i> change, and a GDD nobody updates is worse than none. When a playtest kills an idea, the NOT-list grows and the date bumps."},{id:"gdd-to-spec",type:"concept",eyebrow:"From wish to work order",statement:"Spec = testable sentences.",diagram:'<div class="dg"><div class="dg-cols-2"><div class="dg-card"><div class="dg-card__title">the GDD says</div><div class="dg-card__rows"><div class="dg-card__row">&lsquo;the shop is a breath&rsquo; <span class="dg-card__row__note">a feeling — true, but unbuildable</span></div><div class="dg-card__row">&lsquo;hits are crunchy&rsquo; <span class="dg-card__row__note">which system does that?</span></div></div></div><div class="dg-card"><div class="dg-card__title">the spec says</div><div class="dg-card__rows"><div class="dg-card__row">shop freezes the battlefield <span class="dg-card__row__note">context stack, Unit 10</span></div><div class="dg-card__row">Esc resumes, nothing lost <span class="dg-card__row__note">you can test this</span></div><div class="dg-card__row">hit = shake 8.0 + 4-frame freeze <span class="dg-card__row__note">numbers, not vibes</span></div></div></div></div></div>',caption:"The spec is the bridge between design intent and code — every line checkable, most lines naming their data. If you can't imagine the test, it's still a wish; push until it breaks into sentences."},{id:"spec-example",type:"code",language:"python",eyebrow:"Derived from the GDD, checkable by anyone",heading:"The shop, specced",code:`SPEC: shop   (from GDD: "a breath between waves")

states  playing -> shop on level end -> playing
input   shop swallows ALL input while open
offer   draft(pool, 3), no dupes, rarity-weighted
data    upgrades, prices, growth: settings.py only
feel    cards deal in staggered, ease-out-back

accept  [ ] Esc closes shop, run intact
        [ ] buying applies the stat same frame
        [ ] can't afford = greyed, not hidden`,caption:"Ten lines specify the whole feature: states, input policy, where the data lives, feel notes, and acceptance boxes you can literally tick. Write this <i>before</i> building and the building is mostly typing."},{id:"docs-programmers",type:"concept",eyebrow:"Reader #1 — the next programmer",statement:"Document the seams, not the lines.",diagram:`<div class="dg dg-center"><div class="dg-card" style="min-width:1200px;"><div class="dg-card__title">what the next dev actually needs</div><div class="dg-card__rows"><div class="dg-card__row">a README map <span class="dg-card__row__note">every file, one line: what lives here</span></div><div class="dg-card__row">docstrings on public functions <span class="dg-card__row__note">and on the weird 10%</span></div><div class="dg-card__row">comments answer WHY <span class="dg-card__row__note">the code already says what</span></div><div class="dg-card__row">how to run it <span class="dg-card__row__note">uv run main.py — don't make them guess</span></div><div class="dg-card__row dg-card__row--accent">naming does the rest <span class="dg-card__row__note">if you must explain WHAT, rename instead</span></div></div></div></div>`,caption:"Nobody reads a wall of per-line comments — they read the map, jump to the seam, and trust the names. Spend words where code can't speak: intent, constraints, and the traps you already fell into."},{id:"docstring-example",type:"code",language:"python",eyebrow:"Words where the code can't speak",heading:"A docstring that earns its keep",code:`def apply_effect(player, effect):
    """Apply one upgrade effect to the player.

    effect: {"stat": str, "op": "+"|"*", "value": n}
    Ops resolve against BASE stats (see Unit 10),
    so two +25% buys don't compound.
    multishot hard-caps at MAX_MULTISHOT.

    Weird 10%: "heal" is not a stat - it routes
    to player.heal() before the generic path.
    """`,caption:"Contract, constraints, and the trap — nothing about <i>how</i> the body works; the body says that. A teammate reading only docstrings could use this function correctly. That's the test of a good one."},{id:"docs-designers",type:"concept",eyebrow:"Reader #2 — the designer (incl. future-you)",statement:"The data is the interface.",diagram:'<div class="dg dg-center"><div class="dg-card" style="min-width:1240px;"><div class="dg-card__title">you built this in Unit 10 — now label it</div><div class="dg-card__rows"><div class="dg-card__row">settings.py is the control panel <span class="dg-card__row__note">every knob already lives there</span></div><div class="dg-card__row">each constant: unit + effect + sane range <span class="dg-card__row__note">what happens at 0.4? at 0.8?</span></div><div class="dg-card__row">one worked example <span class="dg-card__row__note">add an upgrade, touch zero code</span></div><div class="dg-card__row dg-card__row--accent">a designer tunes without asking you <span class="dg-card__row__note">the whole point of data-driven</span></div></div></div></div>',caption:"Design-for-change gave the designer knobs; this gives them the legend. If the comment says what breaks at each end of the range, a non-programmer can balance your game — and so can you, in week 12, at 2 a.m."},{id:"legend-example",type:"code",language:"python",eyebrow:"The legend on the knobs",heading:"settings.py, annotated for tuning",code:`# settings.py - the designer's control panel

CURRENCY_RATE = 0.6   # coins per score point.
    # THE economy lever. 0.4 starves the shop,
    # 0.8 maxes the player out by level 3.

PLAYER_CADENCE = 30   # frames between shots.
    # lower = faster. Under 10 melts bosses.

BOSS_HP_MULT = 50     # HP = MULT * level**1.4
    # tune the exponent to punish late levels
    # without touching level 1.`,caption:"Unit, effect, and what breaks at each end — the comment carries what the name can't, and the settings file becomes the tuning manual. The reference implementation's <code>settings.py</code> is written exactly like this. Steal the style."},{id:"tests-as-docs",type:"code",language:"python",eyebrow:"Documentation that can't rot",heading:"A test is a doc that fails when it lies",code:`# test_shop.py  -  run with: uv run pytest

def test_multishot_caps():
    p = make_player()
    for _ in range(99):
        apply_effect(p, {"stat": "multishot",
                         "op": "+", "value": 1})
    assert p.stats["multishot"] == MAX_MULTISHOT

def test_draft_never_duplicates():
    cards = draft(UPGRADES, 3)
    assert len(set(c["id"] for c in cards)) == 3`,caption:"Each test reads as a sentence about the rules: &lsquo;multishot caps&rsquo;, &lsquo;the draft never duplicates&rsquo;. A README can drift from the code silently; this documentation goes <b>red</b> the moment it stops being true."},{id:"dialect",type:"concept",eyebrow:"Why no two studios document alike",statement:"Every team grows its own language.",diagram:`<div class="dg dg-center"><div class="dg-card" style="min-width:1240px;"><div class="dg-card__title">conventions are compressed memory</div><div class="dg-card__rows"><div class="dg-card__row">you already speak one <span class="dg-card__row__note">the seam, the knob, the NOT-list, juice — this course's dialect</span></div><div class="dg-card__row">studios differ wildly <span class="dg-card__row__note">one-pagers, RFCs, Carmack's public .plan files, wiki gardens</span></div><div class="dg-card__row">a shared word is a free doc <span class="dg-card__row__note">&lsquo;that needs a feel pass&rsquo; — nobody opens a wiki</span></div><div class="dg-card__row dg-card__row--accent">form is negotiable, function isn't <span class="dg-card__row__note">fit the doc to the team, never the reverse</span></div></div></div></div>`,caption:"Join a team and you'll inherit a dialect; found one and you'll invent it. Templates off the internet are training wheels — the real skill is noticing what your team keeps re-explaining, and writing that down in words you already share."},{id:"devlog",type:"concept",eyebrow:"The 5-minute habit",statement:"Write it down while it's true.",diagram:`<div class="dg dg-center"><div class="dg-card" style="min-width:1200px;"><div class="dg-card__title">devlog.md — while it's fresh</div><div class="dg-card__rows"><div class="dg-card__row">memory has a half-life of days <span class="dg-card__row__note">the <i>what</i> survives; the <i>why</i> evaporates</span></div><div class="dg-card__row">decisions AND dead ends <span class="dg-card__row__note">dead ends are tomorrow's shortcuts</span></div><div class="dg-card__row">minimum: a dated line + a screenshot <span class="dg-card__row__note">scale up when the stakes do</span></div><div class="dg-card__row dg-card__row--accent">writing the why forces having one <span class="dg-card__row__note">the log improves the work it records</span></div></div></div></div>`,caption:"Reconstructed the night before a deadline, a dev log is fiction; written at the time, it's evidence. And when the documentation PDF is due, you'll be pasting and trimming, not inventing a past."},{id:"workshop",type:"live",heading:"Workshop — one juice pass, one paper pass",items:["<b>Juice:</b> pick one row from the hour map — damage numbers, the impact stack, or dealing cards — and get it into &Uuml;004 now.","<b>vary() sweep:</b> five minutes. Damage, particle life, spawn timing, SFX volume. Feel the difference.","<b>GDD:</b> one page, current truth, dated. Ten minutes — and the NOT-list is mandatory.","<b>README map:</b> every file, one line. If a file needs three lines, it wants splitting.","<b>Devlog entry #1:</b> what changed today, why, one screenshot."]},{id:"next-week",type:"bullets",heading:"For next week",lead:"Unit 12 is polish, presentation and submission prep. Arrive with:",items:["<b>&Uuml;004 feature-complete</b> — shop, draft, scaling, boss. Rough is fine; not building is not.","<b>GDD + README in the repo</b> — I'll ask to see them.","<b>One feel pass shipped</b> — something on screen that wasn't there today.","<b>Your questions collected</b> — last session before deadline week."]},{id:"questions",type:"question",eyebrow:"Before you go",question:"Questions"}],Wt={meta:Nt,slides:Ot},Dt={id:"12",number:"12",tag:"WRAP-UP",total:"8"},Ft=[{id:"section",type:"section",number:"12",kicker:"Unit",title:"Polish & Presentation"},{id:"checkin",type:"question",eyebrow:"Check-in",question:"Ü004 is due, the game follows on the 17th. What's the one thing still standing between you and the zip?"},{id:"today",type:"concept",eyebrow:"The last session",statement:"No new material today.",diagram:'<div class="dg dg-center"><div class="dg-card" style="min-width:1150px;"><div class="dg-card__title">today</div><div class="dg-card__rows"><div class="dg-card__row">Q&amp;A <span class="dg-card__row__note">the sharp questions &mdash; the whole room benefits</span></div><div class="dg-card__row">open workshop <span class="dg-card__row__note">build time, last bugs, doc passes</span></div><div class="dg-card__row dg-card__row--accent">office hours <span class="dg-card__row__note">1:1 &mdash; grading, exercises, anything</span></div></div></div></div>',caption:"We covered practically everything. After today it's email — spend the room while you have it."},{id:"recap",type:"concept",eyebrow:"Twelve weeks, quickly",statement:"You built all of this.",diagram:'<div class="dg"><div class="dg-cols-2"><div class="dg-card"><div class="dg-card__title">the concepts</div><div class="dg-card__rows"><div class="dg-card__row">variables, loops, functions</div><div class="dg-card__row">dictionaries &amp; data files</div><div class="dg-card__row">classes &amp; inheritance</div><div class="dg-card__row">exceptions &amp; debugging</div><div class="dg-card__row">state machines</div></div></div><div class="dg-card"><div class="dg-card__title">the game</div><div class="dg-card__rows"><div class="dg-card__row">the game loop</div><div class="dg-card__row">physics &amp; collision</div><div class="dg-card__row">assets &amp; game objects</div><div class="dg-card__row">the shop &mdash; data, not code</div><div class="dg-card__row dg-card__row--accent">juice &amp; the paper trail</div></div></div></div></div>',caption:"Every row lives somewhere in your RealFakeGame. If you can point at where, you're ready for deadline week."},{id:"the-zip",type:"bullets",heading:"The zip — due 17.07.",lead:"Grading: game 40 &middot; exercises 40 &middot; documentation 20. One zip, max 2.5 GB:",items:["<b>Source + your own assets + a runnable build</b> — not the whole framework folder, not the venv.","<b>Documentation PDF</b> — functions, dev process, concept. Your GDD, README and devlog from Unit 11 feed straight in.","<b>Final versions of Ü001–Ü004</b> — the final versions are what gets graded.","<b>Test it like a stranger:</b> fresh unzip on another machine, run it, nothing missing."]},{id:"qa",type:"question",eyebrow:"Q&A",question:"Ask now — grading, edge cases, “is X allowed?”, deadline logistics. After today it's email."},{id:"workshop",type:"live",heading:"Workshop — office hours",items:["<b>Submission dry-run:</b> zip it now, unzip somewhere else, run it — the deadline-week panic, defused today.","<b>Doc pass:</b> README map + one-page GDD, current truth, dated.","<b>Last bugs:</b> shout — a live bug is still everyone's lesson.","<b>Office hours, 1:1</b> — exercises, grading, the question you didn't want to ask the room."]},{id:"thanks",type:"title",eyebrow:"That's the semester",title:"Thank you.",subtitle:"Submission 17.07. — it was a pleasure. Go build things."}],jt={meta:Dt,slides:Ft},Ut=[st,lt,pt,gt,ft,_t,Tt,St,Rt,Lt,Wt,jt],qt=Ut.map(t=>tt(t.meta,t.slides)).join(`
`),ve=document.createElement("deck-root");ve.innerHTML=qt;document.body.appendChild(ve);
