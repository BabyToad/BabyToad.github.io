(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))i(o);new MutationObserver(o=>{for(const s of o)if(s.type==="childList")for(const d of s.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&i(d)}).observe(document,{childList:!0,subtree:!0});function n(o){const s={};return o.integrity&&(s.integrity=o.integrity),o.referrerPolicy&&(s.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?s.credentials="include":o.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function i(o){if(o.ep)return;o.ep=!0;const s=n(o);fetch(o.href,s)}})();function F(e){const t=e.getContext("2d");e.width=e.clientWidth*devicePixelRatio,e.height=e.clientHeight*devicePixelRatio,t.scale(devicePixelRatio,devicePixelRatio);const n=e.clientWidth,i=e.clientHeight;let o=n/2-20,s=i/2-20;const d=5,h={},g=y=>{"wasd".includes(y.key.toLowerCase())&&(h[y.key.toLowerCase()]=y.type==="keydown",y.preventDefault())};window.addEventListener("keydown",g),window.addEventListener("keyup",g);let w;function m(){h.a&&(o-=d),h.d&&(o+=d),h.w&&(s-=d),h.s&&(s+=d),o=Math.max(0,Math.min(o,n-40)),s=Math.max(0,Math.min(s,i-40)),t.fillStyle="#0b0d10",t.fillRect(0,0,n,i),t.fillStyle="#ff6b35",t.fillRect(o,s,40,40),w=requestAnimationFrame(m)}return m(),()=>{cancelAnimationFrame(w),window.removeEventListener("keydown",g),window.removeEventListener("keyup",g)}}const O={"moving-square":F};let L=null;function Y(e){if(L&&(L(),L=null),!e)return;const t=e.querySelector(".demo__canvas"),n=e.getAttribute("data-demo");if(!t||!n)return;const i=O[n];if(!i){console.warn(`[demo] unknown demo: ${n}`);return}L=i(t)||null}let C=null;function B(e){if(C&&(C(),C=null),!(e!=null&&e.hasAttribute("data-interactive-code")))return;const t=e.querySelector(".icode");if(!t)return;const n=[...e.querySelectorAll(".icode__line[data-region]")],i=[...e.querySelectorAll(".icode__ann[data-region]")],o=[...e.querySelectorAll(".icode-tip")],s=[...new Set(n.map(r=>r.dataset.region).filter(Boolean))],d=new Set;let h=null,g=null,w=!1,m=null;function y(){const r=h!==null;t.classList.toggle("icode--has-active",r),t.classList.toggle("icode--has-revealed",d.size>0);for(const c of n){const u=c.dataset.region;u&&(c.classList.toggle("icode__line--active",u===h),c.classList.toggle("icode__line--revealed",d.has(u)))}for(const c of i){const u=c.dataset.region;c.classList.toggle("icode__ann--active",u===h),c.classList.toggle("icode__ann--revealed",d.has(u))}}function k(){w||d.size>=s.length&&(w=!0,t.classList.add("icode--complete"),setTimeout(()=>t.classList.remove("icode--complete"),900))}y();function _(r){var c,u;if(!((c=r.target)!=null&&c.isContentEditable||/input|textarea/i.test((u=r.target)==null?void 0:u.tagName))){if(r.key>="1"&&r.key<="9"){const b=parseInt(r.key)-1;if(b>=s.length)return;r.preventDefault();const p=s[b];h===p?(h=null,g=null):(d.add(p),h=p,g=p,k()),y();return}if(r.key==="0"||r.key==="Escape"){(h||g)&&(r.preventDefault(),h=null,g=null,y());return}}}addEventListener("keydown",_);const $=[];for(const r of n){const c=r.dataset.region;if(!c)continue;const u=()=>{!g&&d.has(c)&&(h=c,y())},b=()=>{g||(h=null,y())};r.addEventListener("mouseenter",u),r.addEventListener("mouseleave",b),$.push({el:r,events:{mouseenter:u,mouseleave:b}})}for(const r of i){const c=r.dataset.region,u=()=>{!g&&d.has(c)&&(h=c,y())},b=()=>{g||(h=null,y())},p=()=>{g===c?(g=null,h=null):(d.add(c),g=c,h=c,k()),y()};r.addEventListener("mouseenter",u),r.addEventListener("mouseleave",b),r.addEventListener("click",p),$.push({el:r,events:{mouseenter:u,mouseleave:b,click:p}})}function R(r){A();const c=r.dataset.tipSig||"",u=r.dataset.tipDesc||"",b=r.dataset.tipUrl||"";if(!c&&!u)return;const p=document.createElement("div");p.className="icode-tooltip",p.innerHTML=[c?`<code class="icode-tooltip__sig">${j(c)}</code>`:"",u?`<div class="icode-tooltip__desc">${j(u)}</div>`:"",b?`<a class="icode-tooltip__link" href="${j(b)}" target="_blank" rel="noopener">→ docs</a>`:""].filter(Boolean).join(""),t.appendChild(p),t.style.position="relative";let S=0,E=0,x=r;for(;x&&x!==t;)S+=x.offsetTop,E+=x.offsetLeft,x=x.offsetParent;p.style.left=Math.max(0,E)+"px",p.style.visibility="hidden";const q=p.offsetHeight,D=p.offsetWidth;S-q-10<0?(p.style.top=S+r.offsetHeight+8+"px",p.classList.add("icode-tooltip--below")):p.style.top=S-q-8+"px";const M=t.offsetWidth-D-8;parseInt(p.style.left)>M&&(p.style.left=Math.max(0,M)+"px"),p.style.visibility="",m=p}function A(){m&&(m.remove(),m=null)}for(const r of o){const c=()=>R(r),u=()=>A();r.addEventListener("mouseenter",c),r.addEventListener("mouseleave",u),$.push({el:r,events:{mouseenter:c,mouseleave:u}})}C=()=>{removeEventListener("keydown",_);for(const r of $)for(const[c,u]of Object.entries(r.events))r.el.removeEventListener(c,u);A(),h=null,g=null,t.classList.remove("icode--has-active","icode--has-revealed","icode--complete");for(const r of n)r.classList.remove("icode__line--active","icode__line--revealed");for(const r of i)r.classList.remove("icode__ann--active","icode__ann--revealed")}}function j(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const G=localStorage.getItem("deck-theme")||"dark";document.documentElement.setAttribute("data-theme",G);class K extends HTMLElement{connectedCallback(){if(this._wired)return;this._wired=!0;const t=document.createElement("div");for(t.className="deck__stage";this.firstChild;)t.appendChild(this.firstChild);this.appendChild(t),this._stage=t,this._collectSlides(),this._buildFooter(),this._buildHelp(),this._index=this._readHashIndex(),this._show(this._index),this._scale(),addEventListener("resize",()=>this._scale()),addEventListener("keydown",n=>this._onKey(n)),addEventListener("hashchange",()=>this._show(this._readHashIndex()))}_collectSlides(){const n=new URLSearchParams(location.search).get("unit");let i=Array.from(this._stage.querySelectorAll(".slide"));n&&(i=i.filter(o=>o.getAttribute("data-unit")===n),this._stage.querySelectorAll(".slide").forEach(o=>{i.includes(o)||o.remove()})),this._slides=i}_buildFooter(){const t=document.createElement("div");t.className="deck__footer",t.innerHTML=`
...
`).length:0;return`${f(e,t,`slide--demo${i?" slide--demo-with-code":""}`,`data-demo="${n}"`)}
    ...
    </div>`},code(e,t){const n=(e.language||"python").toLowerCase(),i=n==="python"?I(e.code||""):a(e.code||""),o=N(e.code||"");return`${f(e,t,`slide--code slide--code-${o}`)}
      <div class="slide__body">
    ...
    </div>`},"interactive-code"(e,t){const n=e.regions||[],i=e.tooltips||{},o=/^(\s*)#\s*-{3,}\s*.+\s*-{3,}\s*$/,s={};for(const m of n)for(let y=m.lines[0];y<=m.lines[1];y++)s[y]=m;const d=Object.keys(i).sort((m,y)=>y.length-m.length),g=(e.code||"").split(`
`).map((m,y)=>{const k=y+1,_=s[k];let $=I(m)||" ";for(const u of d){const b=u.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),p=i[u],E=typeof p=="object"?`data-tip-sig="${a(p.sig||"")}" data-tip-desc="${a(p.desc||"")}" data-tip-url="${a(p.url||"")}"`:`data-tip-sig="" data-tip-desc="${a(p)}" data-tip-url=""`;$=$.replace(new RegExp(b),`<span class="icode-tip" ${E}>${u}</span>`)}const R=_?_.id:"",A=_?_.color:"transparent";return`<div class="${`icode__line${o.test(m)?" icode__line--divider":""}`}" data-region="${R}" data-line="${k}" style="--rc:${A}"><span class="icode__line-num">${k}</span>${$}</div>`}).join(""),w=n.map((m,y)=>{const k=(m.links||[]).map(_=>`<a href="${a(_.url)}" target="_blank" rel="noopener">→ ${a(_.label)}</a>`).join(" ");return`
      ...
      <div class="slide__body">
        <div class="icode">
          ...
          <div class="icode__cols">
            <pre class="icode__code">${g}</pre>
            <div class="icode__annotations">${w}</div>
          </div>
          ${e.subtitle?`<div class="icode__subtitle" ${l(t.id,e.id,"subtitle")}>${a(e.subtitle)}</div>`:""}
        </div>
      </div>
    </div>`},"annotated-code"(e,t){const n=I(e.code||""),i=N(e.code||""),o=(e.annotations||[]).map((s,d)=>`<li ${l(t.id,e.id,`annotations[${d}]`)}>${T(s)}</li>`).join("");return`${f(e,t,`slide--annotated-code slide--acode-${i}`)}
      <div class="slide__body">
        <div class="acode">
...
ALU: is score greater than or equal to 90?
  If yes → Screen: write "A"
ALU: is score greater than or equal to 80?
  If yes → Screen: write "B"
...
if score >= 90:
    ...
    print("C")`,caption:"ALU yes/no → <b>if/elif/else</b>. The Interpreter picks ONE path and skips the rest.",language:"python"},{id:"round3-script",type:"code",eyebrow:"Round 3 — Loops",heading:"New script. Same team.",code:`Memory: create "total" with value 0

Counter: count from 1 to 5
  Each time:
    ALU: what is total plus the current count?
    Memory: update "total" with the answer

Screen: write the value of total`,caption:"",language:"text"},{id:"round3-reveal",type:"code",eyebrow:"Round 3 — in Python",heading:"What you just did.",code:`total = 0

for i in range(1, 6):
    total = total + i

print(total)`,caption:"The answer is 15. Now imagine counting to 32,482. The computer runs it in microseconds. You'd be here all semester.",language:"python"},{id:"round4-setup",type:"bullets",heading:"Round 4 — The Function Call",lead:"Teams recombine. One team is the program. The other is the function.",items:["<b>Team A:</b> you are the program. You have a script with a function CALL.","<b>Team B:</b> you have NO script yet. You wait.",`Team A's Interpreter reads the <b>define</b> block. They don't run it — they walk the recipe to Team B's Interpreter: "Here's a recipe called [name]. Don't do anything until I call you."`,"Team B reads the recipe, assigns roles, prepares. But does <b>NOT</b> act.",'When Team A hits the <b>call</b> line: write argument values on index cards, hand them to Team B. <b>"Calling [name] with these values."</b>',"Team A <b>sits on their hands</b>. Team B runs.",'When Team B finishes: walk a card back to Team A. <b>"Returning [value]."</b>',"Team A resumes."]},{id:"round4-script-a",type:"code",eyebrow:"Round 4 — Team A (The Program)",heading:"Team A's script.",code:`Define a recipe called "clamp".
...
print(result)`,caption:"Team B's cards were theirs. Team A never saw them. That's <b>scope</b> — local variables exist only inside the function. The <b>def</b> block is read but not run until called.",language:"python"},{id:"game-debrief",type:"bullets",heading:"What just happened",lead:"You were a computer. Each role maps to a real part.",items:["<b>Interpreter</b> → Control Unit. Reads instructions, directs everything.","<b>Memory</b> → RAM. Named slots that hold values.","<b>ALU</b> → Arithmetic Logic Unit. Math and decisions. One chip, two jobs.","<b>Counter</b> → Python's <b>range()</b> iterator. In a real CPU, loops are built from the other parts — a variable, a comparison, and a jump. Python wraps that up for you.","<b>Screen</b> → Standard output (<b>print()</b>). The only thing the outside world sees.","What was annoying? What was slow? What was boring? <b>Those are the things computers are good at.</b>"]},{id:"uebung",type:"bullets",heading:"Uebung 001",lead:"Due next Friday. 10 Punkte.",items:["<b>Schleifen (5P):</b> Zaehle alle Ganzzahlen zwischen 437 und 32482 zusammen. Print das Ergebnis.","<b>Funktionen (5P):</b> Schreibe eine Funktion mit zwei float-Parametern. Teile den kleineren durch den groesseren. Wenn gleich: gib 1 zurueck.","Repo: <b>github.com/BabyToad/macromedia-game-programming-ss26</b>"]},{id:"questions",type:"question",eyebrow:"Before you go",question:"Questions"}],re={meta:se,slides:ae},le=[oe,re],de=le.map(e=>J(e.meta,e.slides)).join(`
`),H=document.createElement("deck-root");H.innerHTML=de;document.body.appendChild(H);
