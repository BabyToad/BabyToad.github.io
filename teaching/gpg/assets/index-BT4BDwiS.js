...
`;function Ye(){if(document.getElementById("mario-jump-styles"))return;const t=document.createElement("style");t.id="mario-jump-styles",t.textContent=qe,document.head.appendChild(t)}function He(t){Ye();const e=t.getContext("2d"),o=Math.max(1,Math.min(3,window.devicePixelRatio||1));let n=0,s=0;function r(){const a=t.getBoundingClientRect();a.width<1||a.height<1||(n=a.width,s=a.height,t.width=Math.round(n*o),t.height=Math.round(s*o))}const p=new ResizeObserver(r);p.observe(t),r();const f=new Image;f.src="img/claude-sprite.png";let x=!1;f.onload=()=>{x=!0};const $=new Image;$.src="img/playground-backdrop.png";let k=!1;$.onload=()=>{k=!0};const v={gravity:.65,jumpStrength:13.5,moveSpeed:5,airControl:1,realisticMode:!1,variableJump:!0,coyoteTime:!0,jumpBuffer:!0,apexHang:!0,asymmetricGravity:!0,fastFall:!0},_={...v},F=["variableJump","coyoteTime","jumpBuffer","apexHang","asymmetricGravity","fastFall"],i={x:80,y:470-H,vx:0,vy:0,facing:1,squash:1},l={onGround:!1,coyote:0,buffer:0,currentGravity:v.gravity,jumpPressedThisFrame:!1,releasedJumpInRise:!1},A={vy:new Float32Array(G),ay:new Float32Array(G),y:new Float32Array(G),head:0};function c(){A.vy[A.head]=i.vy,A.ay[A.head]=l.currentGravity,A.y[A.head]=i.y,A.head=(A.head+1)%G}function m(){i.x=80,i.y=470-H,i.vx=0,i.vy=0,i.facing=1,i.squash=1,l.onGround=!1,l.coyote=0,l.buffer=0,l.releasedJumpInRise=!1,A.vy.fill(0),A.ay.fill(0),A.y.fill(i.y),A.head=0}const u={left:!1,right:!1,jump:!1,down:!1},L={arrowleft:"left",a:"left",arrowright:"right",d:"right",arrowup:"jump",w:"jump"," ":"jump",arrowdown:"down",s:"down"};let b=!1;function W(a){if(!b)return;const y=a.key.toLowerCase();if(y==="r"&&a.type==="keydown"){m(),a.preventDefault(),a.stopImmediatePropagation();return}const g=L[y];if(!g)return;const T=a.type==="keydown";g==="jump"&&(T&&!u.jump&&(l.jumpPressedThisFrame=!0),!T&&u.jump&&i.vy<0&&(l.releasedJumpInRise=!0)),u[g]=T,a.preventDefault(),a.stopImmediatePropagation()}window.addEventListener("keydown",W,!0),window.addEventListener("keyup",W,!0),t.setAttribute("tabindex","0"),t.style.outline="none";const z=()=>{b=!0,t.style.boxShadow="inset 0 0 0 2px rgba(255,107,53,0.6)"},D=()=>{b=!1,t.style.boxShadow="",u.left=u.right=u.jump=u.down=!1};t.addEventListener("focus",z),t.addEventListener("blur",D);const X=()=>t.focus();t.addEventListener("pointerdown",X);function q(a,y,g){return`<label class="pg-toggle">
      <input type="checkbox" data-cfg="${a}" ${v[a]?"checked":""} />
      <span class="pg-toggle__label">${y}</span>
      <span class="pg-toggle__desc">${g}</span>
    </label>`}function J(a,y,g,T,S,R=E=>E.toFixed(2)){return`<label class="pg-slider">
    ...
    <div class="pg-section">
      ...
      ${J("gravity",.1,2,.05,"Gravity")}
      ${J("jumpStrength",5,22,.5,"Jump strength",a=>a.toFixed(1))}
      ${J("moveSpeed",1,10,.5,"Move speed",a=>a.toFixed(1))}
      ${J("airControl",0,1,.05,"Air control")}
    </div>
    <div class="pg-section">
  ...
  `;const Y={vx:C.querySelector("[data-stat=vx]"),vy:C.querySelector("[data-stat=vy]"),ay:C.querySelector("[data-stat=ay]"),ground:C.querySelector("[data-stat=ground]"),coyote:C.querySelector("[data-stat=coyote]"),buffer:C.querySelector("[data-stat=buffer]")};C.querySelectorAll("input[data-cfg]").forEach(a=>{a.addEventListener("input",()=>{const y=a.dataset.cfg;if(a.type==="checkbox"){if(_[y]=a.checked,y==="realisticMode"&&a.checked&&F.forEach(g=>{_[g]=!1;const T=C.querySelector(`input[data-cfg="${g}"]`);T&&(T.checked=!1)}),F.includes(y)&&a.checked&&_.realisticMode){_.realisticMode=!1;const g=C.querySelector('input[data-cfg="realisticMode"]');g&&(g.checked=!1)}}else{const g=parseFloat(a.value);_[y]=g;const T=C.querySelector(`.pg-slider__value[data-for="${y}"]`);T&&(T.textContent=Number.isInteger(g*2)?g.toFixed(1):g.toFixed(2))}})}),C.querySelector(".pg-reset").addEventListener("click",m);const ee=t.closest(".demo__split");ee.classList.add("playground-wrap"),ee.appendChild(C);function he(a,y,g,T,S,R,E,w){return a<S+E&&a+g>S&&y<R+w&&y+T>R}function ae(a,y,g){return a+(y-a)*g}function ke(a){let y=0;u.left&&(y-=_.moveSpeed),u.right&&(y+=_.moveSpeed);const g=l.onGround?1:_.airControl;i.vx=ae(i.vx,y,Math.min(1,.3*g*a)),Math.abs(y)>.2&&(i.facing=Math.sign(y)),l.jumpPressedThisFrame&&(l.buffer=_.jumpBuffer?Ne:1),l.buffer=Math.max(0,l.buffer-a);const T=l.onGround||_.coyoteTime&&l.coyote>0;l.buffer>0&&T&&(i.vy=-_.jumpStrength,l.buffer=0,l.coyote=0,l.onGround=!1,l.releasedJumpInRise=!1,i.squash=1.2),_.variableJump&&l.releasedJumpInRise&&i.vy<0&&(i.vy*=Me,l.releasedJumpInRise=!1),u.jump||(l.releasedJumpInRise=!1);let S=_.gravity;const R=!_.realisticMode;R&&_.apexHang&&Math.abs(i.vy)<We&&(S*=Oe),R&&_.asymmetricGravity&&i.vy>0&&(S*=Ue),R&&_.fastFall&&u.down&&i.vy>0&&(S*=De),l.currentGravity=S,i.vy+=S*a,i.vy>18&&(i.vy=18),i.x+=i.vx*a;for(const[w,I,P,O]of re)he(i.x,i.y,Q,H,w,I,P,O)&&(i.vx>0?i.x=w-Q:i.vx<0&&(i.x=w+P),i.vx=0);const E=l.onGround;l.onGround=!1,i.y+=i.vy*a;for(const[w,I,P,O]of re)if(he(i.x,i.y,Q,H,w,I,P,O)){if(i.vy>0){if(i.y=I-H,l.onGround=!0,!E){const K=Math.min(1,i.vy/16);i.squash=ae(1,.7,K)}}else i.vy<0&&(i.y=I+O);i.vy=0}E&&!l.onGround&&i.vy>=0?l.coyote=je:l.coyote>0&&!l.onGround?l.coyote=Math.max(0,l.coyote-a):l.onGround&&(l.coyote=0),i.squash=ae(i.squash,1,Math.min(1,.2*a)),(i.y>B+200||i.x<-300||i.x>M+300)&&m(),l.jumpPressedThisFrame=!1,c()}let _e=0;function xe(){if(++_e%4!==0)return;const a=y=>y>=0?"+":"";Y.vx.textContent=`${a(i.vx)}${i.vx.toFixed(2)}`,Y.vy.textContent=`${a(i.vy)}${i.vy.toFixed(2)}`,Y.ay.textContent=`${a(l.currentGravity)}${l.currentGravity.toFixed(2)}`,Y.ground.textContent=l.onGround?"yes":"no",Y.ground.className=l.onGround?"good":"muted",Y.coyote.textContent=l.coyote.toFixed(0),Y.coyote.className=l.coyote>0?"accent":"muted",Y.buffer.textContent=l.buffer.toFixed(0),Y.buffer.className=l.buffer>0?"accent":"muted"}function Te(){if(k){const E=$.width/$.height,w=M/B;let I,P,O,K;E>w?(P=B,I=P*E,O=(M-I)/2,K=0):(I=M,P=I/E,O=0,K=(B-P)/2),e.drawImage($,O,K,I,P)}else{const E=e.createLinearGradient(0,0,0,B);E.addColorStop(0,"#0b0d10"),E.addColorStop(1,"#2a1f1a"),e.fillStyle=E,e.fillRect(0,0,M,B)}for(const[E,w,I,P]of re)e.fillStyle="#b78363",e.fillRect(E,w,I,P),e.fillStyle="#f1c591",e.fillRect(E,w,I,5),e.fillStyle="rgba(0,0,0,0.18)",e.fillRect(E+I-3,w,3,P),e.fillStyle="#8a5a3a",e.fillRect(E,w+5,I,2);e.save();const a=i.x+Q/2,y=i.y+H;e.translate(a,y);let g=i.squash,T=1/i.squash;if(!l.onGround){const E=Math.max(-.15,Math.min(.22,-i.vy/80));g*=1+E,T*=1-E*.6}e.scale(T*i.facing,g);const S=Q*2,R=H*2;x?e.drawImage(f,-S/2,-R*.92,S,R):(e.fillStyle="#d97757",e.fillRect(-Q/2,-H,Q,H)),e.restore()}function Ae(){const a=B,y=12,g=3,T=12,S=(M-y*2-T*(g-1))/g,R=le-y*2;e.fillStyle="rgba(11, 13, 16, 0.85)",e.fillRect(0,a,M,le),e.fillStyle="rgba(255,255,255,0.06)",e.fillRect(0,a,M,1),[{label:"y (position)",data:A.y,color:"#6fb3d2",min:0,max:B,invert:!0,zeroLine:!1},{label:"vy (velocity)",data:A.vy,color:"#ff6b35",min:-22,max:22,invert:!1,zeroLine:!0},{label:"ay (accel/grav)",data:A.ay,color:"#98c379",min:0,max:3,invert:!1,zeroLine:!1}].forEach((w,I)=>{const P=y+I*(S+T);if(e.fillStyle="rgba(255,255,255,0.04)",e.fillRect(P,a+y,S,R),e.font="12px ui-monospace, monospace",e.textBaseline="top",e.fillStyle="rgba(255,255,255,0.55)",e.fillText(w.label,P+8,a+y+6),w.zeroLine){const V=a+y+R*((0-w.min)/(w.max-w.min));e.fillStyle="rgba(255,255,255,0.15)",e.fillRect(P,V,S,1)}e.strokeStyle=w.color,e.lineWidth=1.5,e.beginPath();for(let V=0;V<G;V++){const $e=(A.head+V)%G,Re=w.data[$e],Le=V/(G-1),me=Math.max(0,Math.min(1,(Re-w.min)/(w.max-w.min))),Ce=w.invert?me:1-me,ge=P+Le*S,be=a+y+Ce*R;V===0?e.moveTo(ge,be):e.lineTo(ge,be)}e.stroke();const O=(A.head-1+G)%G,K=w.data[O],ye=Math.max(0,Math.min(1,(K-w.min)/(w.max-w.min))),Se=w.invert?ye:1-ye,Pe=P+S-1,Ie=a+y+Se*R;e.fillStyle=w.color,e.beginPath(),e.arc(Pe,Ie,3,0,Math.PI*2),e.fill()})}function Ee(){if(n<1||s<1)return;e.setTransform(1,0,0,1,0,0),e.clearRect(0,0,t.width,t.height);const a=Math.min(n/M,s/Z),y=(n-M*a)/2,g=(s-Z*a)/2,T=a*o;e.setTransform(T,0,0,T,y*o,g*o),e.imageSmoothingEnabled=!1,Te(),Ae(),b||(e.fillStyle="rgba(0,0,0,0.45)",e.fillRect(0,0,M,Z),e.fillStyle="#ffe6d4",e.font="bold 28px ui-monospace, monospace",e.textAlign="center",e.textBaseline="middle",e.fillText("click to play",M/2,Z/2),e.textAlign="start")}let se,pe=performance.now();function ue(a){const y=Math.min(40,a-pe);pe=a;const g=y/(1e3/60);ke(g),xe(),Ee(),se=requestAnimationFrame(ue)}return se=requestAnimationFrame(ue),()=>{cancelAnimationFrame(se),window.removeEventListener("keydown",W,!0),window.removeEventListener("keyup",W,!0),t.removeEventListener("focus",z),t.removeEventListener("blur",D),t.removeEventListener("pointerdown",X),p.disconnect(),C.remove(),ee&&ee.classList.remove("playground-wrap"),t.style.boxShadow=""}}const Ge={"moving-square":Fe,"mario-jump":He};let te=null;function Be(t){if(te&&(te(),te=null),!t)return;const e=t.querySelector(".demo__canvas"),o=t.getAttribute("data-demo");if(!e||!o)return;const n=Ge[o];if(!n){console.warn(`[demo] unknown demo: ${o}`);return}te=n(e)||null}let ne=null;function ze(t){if(ne&&(ne(),ne=null),!(t!=null&&t.hasAttribute("data-interactive-code")))return;const e=t.querySelector(".icode");if(!e)return;const o=[...t.querySelectorAll(".icode__line[data-region]")],n=[...t.querySelectorAll(".icode__ann[data-region]")],s=[...t.querySelectorAll(".icode-tip")],r=[...new Set(o.map(c=>c.dataset.region).filter(Boolean))],p=new Set;let f=null,x=null,$=!1,k=null;function v(){const c=f!==null;e.classList.toggle("icode--has-active",c),e.classList.toggle("icode--has-revealed",p.size>0);for(const m of o){const u=m.dataset.region;u&&(m.classList.toggle("icode__line--active",u===f),m.classList.toggle("icode__line--revealed",p.has(u)))}for(const m of n){const u=m.dataset.region;m.classList.toggle("icode__ann--active",u===f),m.classList.toggle("icode__ann--revealed",p.has(u))}}function _(){$||p.size>=r.length&&($=!0,e.classList.add("icode--complete"),setTimeout(()=>e.classList.remove("icode--complete"),900))}v();function F(c){var m,u;if(!((m=c.target)!=null&&m.isContentEditable||/input|textarea/i.test((u=c.target)==null?void 0:u.tagName))){if(c.key>="1"&&c.key<="9"){const L=parseInt(c.key)-1;if(L>=r.length)return;c.preventDefault();const b=r[L];f===b?(f=null,x=null):(p.add(b),f=b,x=b,_()),v();return}if(c.key==="0"||c.key==="Escape"){(f||x)&&(c.preventDefault(),f=null,x=null,v());return}}}addEventListener("keydown",F);const i=[];for(const c of o){const m=c.dataset.region;if(!m)continue;const u=()=>{!x&&p.has(m)&&(f=m,v())},L=()=>{x||(f=null,v())};c.addEventListener("mouseenter",u),c.addEventListener("mouseleave",L),i.push({el:c,events:{mouseenter:u,mouseleave:L}})}for(const c of n){const m=c.dataset.region,u=()=>{!x&&p.has(m)&&(f=m,v())},L=()=>{x||(f=null,v())},b=()=>{x===m?(x=null,f=null):(p.add(m),x=m,f=m,_()),v()};c.addEventListener("mouseenter",u),c.addEventListener("mouseleave",L),c.addEventListener("click",b),i.push({el:c,events:{mouseenter:u,mouseleave:L,click:b}})}function l(c){A();const m=c.dataset.tipSig||"",u=c.dataset.tipDesc||"",L=c.dataset.tipUrl||"";if(!m&&!u)return;const b=document.createElement("div");b.className="icode-tooltip",b.innerHTML=[m?`<code class="icode-tooltip__sig">${de(m)}</code>`:"",u?`<div class="icode-tooltip__desc">${de(u)}</div>`:"",L?`<a class="icode-tooltip__link" href="${de(L)}" target="_blank" rel="noopener">→ docs</a>`:""].filter(Boolean).join(""),e.appendChild(b),e.style.position="relative";let W=0,z=0,D=c;for(;D&&D!==e;)W+=D.offsetTop,z+=D.offsetLeft,D=D.offsetParent;b.style.left=Math.max(0,z)+"px",b.style.visibility="hidden";const X=b.offsetHeight,q=b.offsetWidth;W-X-10<0?(b.style.top=W+c.offsetHeight+8+"px",b.classList.add("icode-tooltip--below")):b.style.top=W-X-8+"px";const J=e.offsetWidth-q-8;parseInt(b.style.left)>J&&(b.style.left=Math.max(0,J)+"px"),b.style.visibility="",k=b}function A(){k&&(k.remove(),k=null)}for(const c of s){const m=()=>l(c),u=()=>A();c.addEventListener("mouseenter",m),c.addEventListener("mouseleave",u),i.push({el:c,events:{mouseenter:m,mouseleave:u}})}ne=()=>{removeEventListener("keydown",F);for(const c of i)for(const[m,u]of Object.entries(c.events))c.el.removeEventListener(m,u);A(),f=null,x=null,e.classList.remove("icode--has-active","icode--has-revealed","icode--complete");for(const c of o)c.classList.remove("icode__line--active","icode__line--revealed");for(const c of n)c.classList.remove("icode__ann--active","icode__ann--revealed")}}function de(t){return(t||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const Je=localStorage.getItem("deck-theme")||"dark";document.documentElement.setAttribute("data-theme",Je);class Ke extends HTMLElement{connectedCallback(){if(this._wired)return;this._wired=!0;const e=document.createElement("div");for(e.className="deck__stage";this.firstChild;)e.appendChild(this.firstChild);this.appendChild(e),this._stage=e,this._collectSlides(),this._buildFooter(),this._buildHelp(),this._index=this._readHashIndex(),this._show(this._index),this._scale(),addEventListener("resize",()=>this._scale()),addEventListener("keydown",o=>this._onKey(o)),addEventListener("hashchange",()=>this._show(this._readHashIndex()))}_collectSlides(){const o=new URLSearchParams(location.search).get("unit");let n=Array.from(this._stage.querySelectorAll(".slide"));o&&(n=n.filter(s=>s.getAttribute("data-unit")===o),this._stage.querySelectorAll(".slide").forEach(s=>{n.includes(s)||s.remove()})),this._slides=n}_buildFooter(){const e=document.createElement("div");e.className="deck__footer",e.innerHTML=`
...
`)}function nt(t,e){const o=ot[t.type];return o?o(t,e):`<div class="slide"><div class="slide__body">Unknown slide type: ${d(t.type)}</div></div>`}function h(t,e,o){return`data-edit="${t}/${e}/${o}"`}function ie(t,e,o){return`data-edit-list="${t}/${e}/${o}"`}function N(t,e,o="",n=""){return`<div class="slide ${o}" data-slide-type="${d(t.type)}" data-edit-slide="${d(e.id)}/${d(t.id)}" data-unit="${d(e.id)}" data-tag="${d(e.tag||"")}" data-total="${d(e.total||"")}"${n?" "+n:""}>`}const ot={section(t,e){return`${N(t,e,"slide--section")}
    ...
    </div>`},title(t,e){return`${N(t,e,"slide--title")}
      <div class="slide__body">
    ...
    </div>`},bullets(t,e){const o=(t.items||[]).map((n,s)=>`<li ${h(e.id,t.id,`items[${s}]`)}>${U(n)}</li>`).join("");return`${N(t,e,"slide--bullets")}
      <div class="slide__body">
        <div class="bullets">
    ...
    </div>`},question(t,e){return`${N(t,e,"slide--question")}
      <div class="slide__body">
        <div class="qslide">
    ...
    </div>`},prose(t,e){return`${N(t,e,"slide--prose")}
      <div class="slide__body">
        <div class="prose">
    ...
    </div>`},concept(t,e){const o=t.diagram||"";return`${N(t,e,"slide--concept")}
      <div class="slide__body">
        <div class="concept">
    ...
    </div>`},image(t,e){const o=t.fit==="contain"?"contain":"cover",n=d(t.src||""),s=d(t.alt||"");return`${N(t,e,"slide--image")}
      <div class="slide__body">
...
`).length:0;return`${N(t,e,`slide--demo${n?" slide--demo-with-code":""}`,`data-demo="${o}"`)}
      <div class="slide__body">
        <div class="demo">
    ...
    </div>`},code(t,e){const o=(t.language||"python").toLowerCase(),n=o==="python"?oe(t.code||""):d(t.code||""),s=we(t.code||"");return`${N(t,e,`slide--code slide--code-${s}`)}
      <div class="slide__body">
...
`).map((k,v)=>{const _=v+1,F=r[_];let i=oe(k)||" ";for(const u of p){const L=u.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),b=n[u],z=typeof b=="object"?`data-tip-sig="${d(b.sig||"")}" data-tip-desc="${d(b.desc||"")}" data-tip-url="${d(b.url||"")}"`:`data-tip-sig="" data-tip-desc="${d(b)}" data-tip-url=""`;i=i.replace(new RegExp(L),`<span class="icode-tip" ${z}>${u}</span>`)}const l=F?F.id:"",A=F?F.color:"transparent";return`<div class="${`icode__line${s.test(k)?" icode__line--divider":""}`}" data-region="${l}" data-line="${_}" style="--rc:${A}"><span class="icode__line-num">${_}</span>${i}</div>`}).join(""),$=o.map((k,v)=>{const _=(k.links||[]).map(F=>`<a href="${d(F.url)}" target="_blank" rel="noopener">→ ${d(F.label)}</a>`).join(" ");return`
      ...
      </div>`}).join("");return`${N(t,e,"slide--interactive-code","data-interactive-code")}
      <div class="slide__body">
        <div class="icode">
    ...
    </div>`},"annotated-code"(t,e){const o=oe(t.code||""),n=we(t.code||""),s=(t.annotations||[]).map((r,p)=>`<li ${h(e.id,t.id,`annotations[${p}]`)}>${U(r)}</li>`).join("");return`${N(t,e,`slide--annotated-code slide--acode-${n}`)}
      <div class="slide__body">
        <div class="acode">
    ...
    </div>`},live(t,e){const o=(t.items||[]).map((n,s)=>`<li ${h(e.id,t.id,`items[${s}]`)}>${U(n)}</li>`).join("");return`${N(t,e,"slide--live")}
      <div class="slide__body">
        <div class="live">
